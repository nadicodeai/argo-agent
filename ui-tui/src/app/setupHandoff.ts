import type { RunExternalProcess } from '@argo/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`argo ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching argo: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`argo ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
