...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ARGO_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Argo on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ARGO_TUI_TRUECOLOR=1.
