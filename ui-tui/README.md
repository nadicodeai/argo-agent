...
React + Ink terminal UI for Argo. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ARGO_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.argo/.argo_history` or under `ARGO_HOME`.
...
```text
ui-tui/
  packages/argo-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      argo-ink.d.ts    type declarations for @argo/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ArgoCLI subprocess for slash commands
```
