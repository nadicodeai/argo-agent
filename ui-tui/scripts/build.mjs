...
await build({
  ...
  // Skip the prebuilt @argo/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@argo/ink': resolve(root, 'packages/argo-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The argo_cli launcher
