...
See: https://github.com/NousResearch/argo-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for argo_constants.get_subprocess_home()."""

    def test_returns_none_when_argo_home_unset(self, monkeypatch):
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ARGO_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("ARGO_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        from argo_constants import (
            get_argo_home,
            reset_argo_home_override,
            set_argo_home_override,
        )
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_argo_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_argo_home_override(profile)
        try:
            assert get_argo_home() == profile
            release.set()
            thread.join(timeout=5)
        finally:
            reset_argo_home_override(token)
            release.set()

        assert seen == [str(root)]
        assert get_argo_home() == root
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(argo_home / "home")
    ...
    def test_no_injection_when_argo_home_unset(self, monkeypatch):
        monkeypatch.delenv("ARGO_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["ARGO_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from argo_cli.profiles import _PROFILE_DIRS
