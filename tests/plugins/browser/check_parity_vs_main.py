...
Run from the PR worktree:
    ...
    cd ~/.argo/argo-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.argo/argo-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.argo/argo-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a argo-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a argo-agent checkout"
)
...
# Isolated ARGO_HOME for the config write.
