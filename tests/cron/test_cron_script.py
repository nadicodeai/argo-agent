...
- Path resolution (absolute, relative to ARGO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ARGO_HOME."""
    argo_home = tmp_path / ".argo"
    argo_home.mkdir()
    (argo_home / "cron").mkdir()
    (argo_home / "cron" / "output").mkdir()
    (argo_home / "scripts").mkdir()
    monkeypatch.setenv("ARGO_HOME", str(argo_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ARGO_DIR", argo_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", argo_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", argo_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", argo_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.argo/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ARGO_SESSION_PLATFORM") is None
        assert os.environ.get("ARGO_SESSION_CHAT_ID") is None
        assert os.environ.get("ARGO_SESSION_CHAT_NAME") is None
