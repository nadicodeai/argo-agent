"""
Tests for mcp_serve — Argo MCP server.
...
def _isolate_argo_home(tmp_path, monkeypatch):
    """Redirect ARGO_HOME to a temp directory."""
    monkeypatch.setenv("ARGO_HOME", str(tmp_path))
    try:
        import argo_constants
        monkeypatch.setattr(argo_constants, "get_argo_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking argo_state schema."""
