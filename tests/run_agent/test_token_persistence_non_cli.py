...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    argo_state = ModuleType("argo_state")
    argo_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "argo_state", argo_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Argo"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Argo"
