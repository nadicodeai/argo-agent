...
class TestToolsetConsistency:
    ...
    def test_argo_platforms_share_core_tools(self):
        """All argo-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on argo-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["argo-cli", "argo-telegram", "argo-discord", "argo-whatsapp", "argo-slack", "argo-signal", "argo-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_argo_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("argo-whatsapp")

    def test_argo_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("argo-api-server")
