...
- Signal forwarding runs through the init so ``docker stop`` triggers
  argo's own graceful-shutdown path.
"""
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, argo accumulates zombie
    ...
    assert installed, (
        ...
        "orphaned subprocesses, argo accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into argo,
    ...
    assert routes_through_init, (
        ...
        "If tini is only installed but not wired into ENTRYPOINT, argo "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``argo-ink`` workspace package is
    ...
    assert "ui-tui/packages/argo-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    assert "ui-tui/packages/argo-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled argo-ink workspace package "
        "so ``await import('@argo/ink')`` resolves at runtime."
    )
