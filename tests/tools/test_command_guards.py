...
def _clean_state():
    ...
    saved = {}
    for k in ("ARGO_INTERACTIVE", "ARGO_GATEWAY_SESSION", "ARGO_EXEC_ASK", "ARGO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ARGO_INTERACTIVE"] = "1"
        session_key = os.getenv("ARGO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ARGO_INTERACTIVE or ARGO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
