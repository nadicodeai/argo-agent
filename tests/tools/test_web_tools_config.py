...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_argo_home_override(self, tmp_path):
        """Auth lookup should read from ARGO_HOME/auth.json, not ~/.argo/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".argo").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``argo tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
