...
def test_argo_provider_subclass_exists():
    """ArgoMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ARGO_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ARGO_PROVIDER_CLS is not None
    assert issubclass(_ARGO_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_argo_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ArgoMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ARGO_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ARGO_HOME", str(tmp_path))
    ...
    assert _ARGO_PROVIDER_CLS is not None
    assert isinstance(provider, _ARGO_PROVIDER_CLS)
    assert provider._argo_server_name == "srv"
