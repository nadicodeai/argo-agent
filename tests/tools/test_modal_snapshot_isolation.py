...
def _restore_tool_modules():
    original_argo_home = os.environ.get("ARGO_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_argo_home is None:
            os.environ.pop("ARGO_HOME", None)
        else:
            os.environ["ARGO_HOME"] = original_argo_home
        _reset_modules(("tools", "argo_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    argo_cli = types.ModuleType("argo_cli")
    argo_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["argo_cli"] = argo_cli
    argo_home = tmp_path / "argo-home"
    os.environ["ARGO_HOME"] = str(argo_home)
    sys.modules["argo_cli.config"] = types.SimpleNamespace(
        get_argo_home=lambda: argo_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="argo-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": argo_home / "modal_snapshots.json",
