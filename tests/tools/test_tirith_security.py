...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._argo_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_argo_bin(self):
        """After _INSTALL_FAILED, manual install in ARGO_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        argo_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(argo_bin, "w") as f:
            f.write("#!/bin/sh\n")
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._argo_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == argo_bin
            assert _tirith_mod._resolved_path == argo_bin
            mock_clear.assert_called_once()
...
class TestArgoHomeIsolation:
    def test_argo_bin_dir_respects_argo_home(self):
        """_argo_bin_dir must use ARGO_HOME, not hardcoded ~/.argo."""
        from tools.tirith_security import _argo_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ARGO_HOME": tmpdir}):
            result = _argo_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_argo_home(self):
        """_failure_marker_path must use ARGO_HOME, not hardcoded ~/.argo."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ARGO_HOME": "/custom/argo"}):
            result = _failure_marker_path()
        assert result == "/custom/argo/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ARGO_HOME; verify it's active."""
        argo_home = os.getenv("ARGO_HOME")
        assert argo_home is not None, "ARGO_HOME should be set by conftest"
        assert "argo_test" in argo_home, "Should point to test temp dir"

    def test_get_argo_home_fallback(self):
        """Without ARGO_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_argo_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ARGO_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".argo")
            result = _get_argo_home()
