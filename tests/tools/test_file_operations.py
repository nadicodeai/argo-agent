...
class TestIsWriteDenied:
    ...
    def test_argo_control_files_and_mcp_tokens_denied(self, path):
        """Argo control files and mcp-tokens entries must be write-denied."""
        from argo_constants import get_argo_home
        argo_home = get_argo_home()
        full_path = str(argo_home / path)
    ...
    def test_argo_control_files_traversal_denied(self, path):
    ...
    def test_control_files_protected_in_profile_mode(self, tmp_path, monkeypatch, name):
        ...
        global ~/.argo/{auth.json,config.yaml,webhook_subscriptions.json}
        writable — the same gap PR #15981 fixed for .env.
        """
        # Simulate a profile-mode ARGO_HOME layout:
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__ARGO_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__ARGO_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "ARGO_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__ARGO_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__ARGO_FENCE_a9f7b3__\n"
        )
