...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Argo Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_argo_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("ARGO_HOME", str(tmp_path / ".argo"))

    argo_env = tmp_path / ".argo" / ".env"
    argo_env.parent.mkdir(parents=True)
    argo_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.argo/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
