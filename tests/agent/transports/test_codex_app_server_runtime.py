...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Argo passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("ARGO_HOME", "/users/alice/.argo/profiles/backend-worker")
        monkeypatch.setenv("ARGO_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "ARGO_KANBAN_DB",
            "/users/alice/.argo/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.argo/kanban/boards/smoke"]'
            in cmd
        )
