...
""",
        ...
        from argo_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        argo_home = tmp_path / ".argo"
        argo_home.mkdir()
        (argo_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("ARGO_HOME", str(argo_home))
        from argo_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from argo_cli.plugins import PluginContext, PluginManifest, PluginManager
