...
def test_argo_client_tag_includes_current_version():
    """The client tag must reflect argo_cli.__version__ verbatim."""
    from argo_cli import __version__
    from agent.portal_tags import argo_client_tag

    assert argo_client_tag() == f"client=argo-client-v{__version__}"


def test_argo_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import argo_client_tag

    tag = argo_client_tag()
    assert tag.startswith("client=argo-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import argo_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=argo-agent" in tags
    assert argo_client_tag() in tags
    assert len(tags) == 2
