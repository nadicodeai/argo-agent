"""Tests for argo_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    argo_logging._logging_initialized = False
    ...
    argo_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    argo_logging._logging_initialized = False
    argo_logging.clear_session_context()


@pytest.fixture
def argo_home(tmp_path, monkeypatch):
    """Provide an isolated ARGO_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_argo_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["ARGO_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, argo_home):
        log_dir = argo_logging.setup_logging(argo_home=argo_home)
        assert log_dir == argo_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)
        argo_logging.setup_logging(argo_home=argo_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        argo_logging.setup_logging(argo_home=argo_home, force=True)
    ...
    def test_custom_log_level(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, argo_home):
        argo_logging.setup_logging(
            argo_home=argo_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)
    ...
    def test_writes_to_agent_log(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)

        test_logger = logging.getLogger("test_argo_logging.write_test")
        ...
        agent_log = argo_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)

        test_logger = logging.getLogger("test_argo_logging.warning_test")
        ...
        agent_log = argo_home / "logs" / "agent.log"
        errors_log = argo_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)

        test_logger = logging.getLogger("test_argo_logging.info_test")
    ...
    def test_reads_config_yaml(self, argo_home):
    ...
    def test_explicit_params_override_config(self, argo_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (argo_home / "config.yaml").write_text(yaml.dump(config))

        argo_logging.setup_logging(argo_home=argo_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, argo_home):
        """The custom record factory injects session_tag on all records."""
        argo_logging.setup_logging(argo_home=argo_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_argo_session_injector", False), (
            "Record factory should have _argo_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, argo_home):
        ...
        gw_log = argo_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, argo_home):
    ...
    def test_gateway_log_receives_gateway_records(self, argo_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, argo_home):
    ...
    def test_agent_log_still_receives_all(self, argo_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, argo_home):
        """When session context is set, log lines include [session_id]."""
        argo_logging.setup_logging(argo_home=argo_home)
        argo_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, argo_home):
    ...
    def test_clear_session_context(self, argo_home):
        """After clearing, session tag disappears."""
        argo_logging.setup_logging(argo_home=argo_home)
        argo_logging.set_session_context("xyz789")
        argo_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, argo_home):
        ...
        def thread_a():
            argo_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            argo_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        argo_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        argo_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = argo_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = argo_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in argo_logging.COMPONENT_PREFIXES
        # The gateway component captures both core gateway logs and the
        # argo_plugins facility (plugin-installed gateway adapters log
        # under that prefix).
        assert ("gateway", "argo_plugins") == argo_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = argo_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == argo_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = argo_logging.COMPONENT_PREFIXES["cli"]
        assert "argo_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == argo_logging.COMPONENT_PREFIXES["cron"]


class TestSetupVerboseLogging:
    """setup_verbose_logging() adds a DEBUG-level console handler."""

    def test_adds_stream_handler(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)
        argo_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_argo_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, argo_home):
        argo_logging.setup_logging(argo_home=argo_home)
        argo_logging.setup_verbose_logging()
        argo_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        argo_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = argo_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        argo_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("argo_cli.config.is_managed", return_value=True):
                argo_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, argo_home):
        level, max_size, backup = argo_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, argo_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, argo_home):
        import yaml
        config = {"model": "test"}
        (argo_home / "config.yaml").write_text(yaml.dump(config))
