"""
Tests for timezone support (argo_time module + integration points).
...
def _reset_argo_time_cache():
    """Reset the argo_time module cache (replacement for removed reset_cache)."""
    argo_time._cached_tz = None
    argo_time._cached_tz_name = None
    argo_time._cache_resolved = False


# =========================================================================
# argo_time.now() — core helper
# =========================================================================

class TestArgoTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_argo_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["ARGO_TIMEZONE"] = "Asia/Kolkata"
        result = argo_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["ARGO_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="argo_time"):
            result = argo_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["ARGO_TIMEZONE"] = "Europe/London"
        tz = argo_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["ARGO_TIMEZONE"] = "Not/A/Timezone"
        tz = argo_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When ARGO_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) ARGO_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("ARGO_TIMEZONE=" + os.environ.get("ARGO_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "ARGO_TIMEZONE=NOT_SET" in result["output"], (
            "ARGO_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When ARGO_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("ARGO_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=argo_tz) which shifted
        absolute time when system-local tz != Argo tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_argo_tz(self):
        """Already-aware datetimes should be normalized to Argo tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Argo tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Argo tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Argo is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Argo tz
        ...
        # Use a Argo timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _argo_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["ARGO_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_argo_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Argo timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["ARGO_TIMEZONE"] = "US/Eastern"
        _reset_argo_time_cache()
