...
class TestArgoConstantsFallback:
    """Tests for _argo_home.py fallback when argo_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_argo_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _argo_home.py with argo_constants blocked."""
        monkeypatch.setitem(sys.modules, "argo_constants", None)
        spec = importlib.util.spec_from_file_location("_argo_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_argo_home_env_var(self, monkeypatch, tmp_path):
        """When argo_constants is missing, ARGO_HOME comes from env var."""
        monkeypatch.setenv("ARGO_HOME", str(tmp_path / "custom-argo"))
        module = self._load_helper(monkeypatch)
        assert module.get_argo_home() == tmp_path / "custom-argo"

    def test_fallback_defaults_to_dot_argo(self, monkeypatch):
        """When argo_constants is missing and ARGO_HOME unset, default to ~/.argo."""
        monkeypatch.delenv("ARGO_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_argo_home() == Path.home() / ".argo"

    def test_fallback_ignores_empty_argo_home(self, monkeypatch):
        """Empty/whitespace ARGO_HOME is treated as unset."""
        monkeypatch.setenv("ARGO_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_argo_home() == Path.home() / ".argo"

    def test_fallback_display_argo_home_shortens_path(self, monkeypatch):
        """Fallback display_argo_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ARGO_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_argo_home() == "~/.argo"

    def test_fallback_display_argo_home_profile_path(self, monkeypatch):
        """Fallback display_argo_home() handles profile paths under ~/."""
        monkeypatch.setenv("ARGO_HOME", str(Path.home() / ".argo/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_argo_home() == "~/.argo/profiles/coder"

    def test_fallback_display_argo_home_custom_path(self, monkeypatch):
        """Fallback display_argo_home() returns full path for non-home locations."""
        monkeypatch.setenv("ARGO_HOME", "/opt/argo-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_argo_home() == "/opt/argo-custom"

    def test_delegates_to_argo_constants_when_available(self):
        """When argo_constants IS importable, _argo_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_argo_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import argo_constants
        assert module.get_argo_home is argo_constants.get_argo_home
        assert module.display_argo_home is argo_constants.display_argo_home
