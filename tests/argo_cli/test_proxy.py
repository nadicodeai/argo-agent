"""Tests for the `argo proxy` subcommand and its upstream adapters."""
...
from argo_cli.proxy.adapters import ADAPTERS, get_adapter
from argo_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from argo_cli.proxy.adapters.nous_portal import NousPortalAdapter
from argo_cli.proxy.adapters.xai import XAIGrokAdapter
...
def _write_auth_store(argo_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic ARGO_HOME."""
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # ARGO_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with patch(
        "argo_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("ARGO_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="argo login nous"):
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic ARGO_HOME."""
    auth_path = argo_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_refreshes_current_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("argo_cli.auth.refresh_xai_oauth_pure", fake_refresh)
...
from argo_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("ARGO_HOME", str(tmp_path))
    from argo_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from argo_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from argo_cli.proxy.cli import cmd_proxy_start
