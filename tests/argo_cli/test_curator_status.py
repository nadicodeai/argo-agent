"""Tests for `argo curator status` output.
...
def test_status_uses_last_activity_not_only_last_used(monkeypatch, capsys):
    ...
    import argo_cli.curator as curator_cli
...
def curator_status_env(tmp_path, monkeypatch):
    """Isolated ARGO_HOME with real agent-created skills on disk."""
    home = tmp_path / ".argo"
    skills = home / "skills"
    ...
    from argo_cli import curator as curator_cli
