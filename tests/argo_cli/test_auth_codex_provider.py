"""Tests for Codex auth — tokens stored in Argo auth store (~/.argo/auth.json)."""
...
def _setup_argo_auth(argo_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Argo auth store."""
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_argo_auth(argo_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ARGO_HOME", str(argo_home))
    ...
    monkeypatch.setattr("argo_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    argo_home = tmp_path / "argo"
    _setup_argo_auth(argo_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ARGO_HOME", str(argo_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Argo auth store, not ~/.codex/."""
    argo_home = tmp_path / "argo"
    codex_home = tmp_path / "codex-cli"
    argo_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "argo-at", "refresh_token": "argo-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Argo store
    assert not (codex_home / "auth.json").exists()

    # Argo auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "argo-at"


def test_resolve_returns_argo_auth_store_source(tmp_path, monkeypatch):
    argo_home = tmp_path / "argo"
    _setup_argo_auth(argo_home)
    monkeypatch.setenv("ARGO_HOME", str(argo_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr("argo_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("argo_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
