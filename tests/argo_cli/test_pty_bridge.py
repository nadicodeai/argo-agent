"""Unit tests for argo_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf argo-ok"])
        try:
            output = _read_until(bridge, b"argo-ok")
            assert b"argo-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$ARGO_PTY_TEST\""],
            env={**os.environ, "ARGO_PTY_TEST": "pty-env-works"},
        )
        try:
