"""Tests for IRC gateway configuration via `argo setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Argo install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Argo install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#argo")
            monkeypatch.setenv("IRC_NICKNAME", "argo-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `argo setup gateway` experience with IRC present."""
