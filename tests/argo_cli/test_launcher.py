"""Tests for the top-level `./argo` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./argo` should use `argo_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "argo"
    called = []

    fake_main_module = types.ModuleType("argo_cli.main")

    def fake_main():
        called.append("argo_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "argo_cli.main", fake_main_module)
