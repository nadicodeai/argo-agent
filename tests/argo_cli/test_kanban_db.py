"""Tests for the Kanban DB layer (argo_cli.kanban_db)."""
...
from argo_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated ARGO_HOME with an empty kanban DB."""
    home = tmp_path / ".argo"
    home.mkdir()
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("ARGO_HOME", str(home))
    monkeypatch.delenv("ARGO_KANBAN_DB", raising=False)
    monkeypatch.delenv("ARGO_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("ARGO_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import argo_cli.kanban_db as _kb
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Argo profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="argo-task", assignee="daily")
...
def test_scratch_workspace_created_under_argo_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `argo -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's ARGO_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's ARGO_HOME."""

    def _set_home(self, monkeypatch, tmp_path, argo_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARGO_HOME", str(argo_home))
        monkeypatch.delenv("ARGO_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_argo(
        self, tmp_path, monkeypatch
    ):
        # Standard install: ARGO_HOME == ~/.argo, no profile active.
        default_home = tmp_path / ".argo"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.argo/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.argo/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.argo/kanban.db.
        default_home = tmp_path / ".argo"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # ARGO_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `argo -p coder`).
        monkeypatch.setenv("ARGO_HOME", str(profile_home))
    ...
    def test_docker_custom_argo_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: ARGO_HOME points outside ~/.argo.
        # `get_default_argo_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".argo"`.
        custom_root = tmp_path / "opt" / "argo"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: ARGO_HOME=/opt/argo/profiles/coder;
        # `get_default_argo_root()` walks up to /opt/argo because
    ...
    def test_explicit_override_via_argo_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: ARGO_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".argo"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARGO_HOME", str(profile_home))
        monkeypatch.setenv("ARGO_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARGO_HOME", str(default_home))
        monkeypatch.setenv("ARGO_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # ARGO_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile ARGO_HOME and reads.
        monkeypatch.setenv("ARGO_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_argo_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ARGO_KANBAN_DB pins the file path directly and beats both
        # ARGO_KANBAN_HOME and the `get_default_argo_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARGO_HOME", str(default_home))
        monkeypatch.setenv("ARGO_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ARGO_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows ARGO_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_argo_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # ARGO_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".argo"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("ARGO_HOME", str(default_home))
        monkeypatch.setenv("ARGO_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("ARGO_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows ARGO_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARGO_HOME", str(default_home))
        monkeypatch.setenv("ARGO_KANBAN_DB", "   ")
        monkeypatch.setenv("ARGO_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject ARGO_KANBAN_DB
        # and ARGO_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites ARGO_HOME.
        default_home = tmp_path / ".argo"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["ARGO_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["ARGO_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["ARGO_KANBAN_TASK"] == "t_dispatch_env"
        assert env["ARGO_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see argo_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("argo_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="argo_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'argo kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_argo_argv()
#
# Workers spawned by the dispatcher must use a `argo` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["argo", ...]`
...
def test_resolve_argo_argv_prefers_path_shim(monkeypatch):
    """When `argo` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import argo_cli.kanban_db as kb

    monkeypatch.delenv("ARGO_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/argo")
    argv = kb._resolve_argo_argv()
    assert argv == ["/usr/local/bin/argo"]


def test_resolve_argo_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import argo_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("ARGO_BIN", ".\\argo.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_argo_argv() == [os.path.abspath(".\\argo.exe")]


def test_resolve_argo_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "argo.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("ARGO_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_argo_argv() == [sys.executable, "-m", "argo_cli.main"]


def test_resolve_argo_argv_honors_argo_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like ARGO_BIN lets service managers pin the executable."""
    import shutil
    import argo_cli.kanban_db as kb

    shim = tmp_path / "bin" / "argo"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("ARGO_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_argo_argv() == [str(shim)]


def test_resolve_argo_argv_argo_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare ARGO_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import argo_cli.kanban_db as kb

    cwd_argo = tmp_path / "argo"
    cwd_argo.write_text("wrong\n", encoding="utf-8")
    cwd_argo.chmod(cwd_argo.stat().st_mode | stat.S_IXUSR)
    path_argo = tmp_path / "bin" / "argo"
    path_argo.parent.mkdir()
    path_argo.write_text("right\n", encoding="utf-8")
    path_argo.chmod(path_argo.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_argo.parent))
    monkeypatch.setenv("ARGO_BIN", "argo")

    assert kb._resolve_argo_argv() == [str(path_argo)]


def test_resolve_argo_argv_argo_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare ARGO_BIN does not accept current-directory shadow executables."""
    import sys
    import argo_cli.kanban_db as kb

    (tmp_path / "argo.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_argo_argv_argo_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved ARGO_BIN batch shim is not used as worker argv[0]."""
    import sys
    import argo_cli.kanban_db as kb
...
def test_resolve_argo_argv_argo_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved ARGO_BIN command names do not delegate cwd search to Popen."""
    import sys
    import argo_cli.kanban_db as kb
...
def test_resolve_argo_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m argo_cli.main`.

    Pins the correct module name (NOT `argo` — there is no top-level
    `argo` package). Regression for #23198: the original PR shipped
    `python -m argo` which fails with `No module named argo` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_argo_argv()
    assert argv == [sys.executable, "-m", "argo_cli.main"]


def test_resolve_argo_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `argo_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m argo_cli.main --version`
    ...
    assert "Argo Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="argo_cli.kanban_db"):
        with kb.connect() as conn:
