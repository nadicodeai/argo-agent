"""Tests for the kanban CLI surface (argo_cli.kanban)."""
...
from argo_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`argo kanban list --session <id>` filters by the originating
...
def test_kanban_in_autocomplete_table():
    from argo_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_not_gateway_only():
    ...
    from argo_cli.commands import COMMAND_REGISTRY
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("ARGO_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("ARGO_KANBAN_BOARD") == "beta"
