...
from argo_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from argo_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ARGO_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_argo_ca_bundle_env_falls_back(self, monkeypatch):
        from argo_cli.auth import _resolve_verify

        monkeypatch.setenv("ARGO_CA_BUNDLE", "/nonexistent/argo-ca.pem")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="argo_cli.auth")
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    argo_home = tmp_path / "argo"
    _setup_nous_auth(argo_home, access_token="at-123")
    monkeypatch.setenv("ARGO_HOME", str(argo_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    argo_home = tmp_path / "argo"
    _setup_nous_auth(argo_home, access_token="at-fresh")
    monkeypatch.setenv("ARGO_HOME", str(argo_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("argo_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    argo_home = tmp_path / "argo"
    _setup_nous_auth(argo_home, refresh_token="refresh-old")
    monkeypatch.setenv("ARGO_HOME", str(argo_home))
    ...
    monkeypatch.setattr("argo_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("argo_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `argo model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return argo_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import argo_cli.auth as auth_mod
        import argo_cli.models as models_mod
        import argo_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from argo_cli.auth import PROVIDER_REGISTRY, _login_nous

        argo_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `argo auth add nous`
    ...
    """
    from argo_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Argo is not logged into Nous Portal").
    ...
    # doesn't raise "Argo is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `argo auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Argo must surface an
    ...
    re-authenticate" string led users to report this as a Argo persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from argo_cli.auth import _refresh_access_token
    ...
    assert "argo auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ARGO_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("ARGO_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ARGO_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.argo/shared/`` across CI runs.
    """
    from argo_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("ARGO_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """ARGO_SHARED_AUTH_DIR must redirect the path."""
    from argo_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("ARGO_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from argo_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from argo_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from argo_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `argo auth add nous
    ...
    argo_home = tmp_path / "argo"
    argo_home.mkdir(parents=True, exist_ok=True)
    (argo_home / "auth.json").write_text(
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from argo_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "argo-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different ARGO_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("ARGO_HOME", str(profile_a))
    ...
    # Profile B: fresh ARGO_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("ARGO_HOME", str(profile_b))
