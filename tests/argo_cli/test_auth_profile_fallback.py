...
When ``ARGO_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.argo/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.argo            (has its own auth.json fixture)
    * Profile     -> tmp_path/.argo/profiles/coder   (active, ARGO_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (ARGO_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.argo as the "real home" — which is NOT equal
    # to the ARGO_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    argo_home = tmp_path / "classic"
    argo_home.mkdir()
    monkeypatch.setenv("ARGO_HOME", str(argo_home))

    _write(argo_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from argo_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: ARGO_HOME is set to a custom path that is NOT under
    # ~/.argo/profiles/ — get_default_argo_root() returns ARGO_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from argo_cli.auth import read_credential_pool, write_credential_pool
