"""Tests for the `argo memory reset` CLI command.
...
- Profile-scoped reset (uses ARGO_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ARGO_HOME with memory files."""
    argo_home = tmp_path / ".argo"
    memories = argo_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ARGO_HOME", str(argo_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nArgo repo is at ~/.argo/argo-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `argo memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        argo_home = tmp_path / ".argo"
        (argo_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ARGO_HOME", str(argo_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ARGO_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        argo_home = tmp_path / ".argo"
        argo_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("ARGO_HOME", str(argo_home))

        # The memories dir won't exist; get_argo_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
