...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ARGO_HOME with .container-mode file."""
    argo_home = tmp_path / ".argo"
    argo_home.mkdir()
    monkeypatch.setenv("ARGO_HOME", str(argo_home))
    monkeypatch.delenv("ARGO_DEV", raising=False)

    container_mode = argo_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=argo-agent\n"
        "exec_user=argo\n"
        "argo_bin=/data/current-package/bin/argo\n"
    )
    return argo_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("argo_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "argo-agent"
    assert info["exec_user"] == "argo"
    assert info["argo_bin"] == "/data/current-package/bin/argo"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("argo_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_argo_dev(container_env, monkeypatch):
    """Returns None when ARGO_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ARGO_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_argo_dev_zero(container_env, monkeypatch):
    """ARGO_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ARGO_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        argo_home = Path(tmpdir) / ".argo"
        argo_home.mkdir()
        (argo_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("argo_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_argo_home": lambda: argo_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ARGO_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=argo-custom\n"
        "exec_user=myuser\n"
        "argo_bin=/opt/argo/bin/argo\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "argo-custom"
    assert info["exec_user"] == "myuser"
    assert info["argo_bin"] == "/opt/argo/bin/argo"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from argo_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "argo"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "argo-agent" in cmd
    assert "/data/current-package/bin/argo" in cmd
    assert "chat" in cmd
