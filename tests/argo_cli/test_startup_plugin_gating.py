...
``argo_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``argo --help``, ``argo version``,
``argo logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``argo --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from argo_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["argo", "-w", "--tui"], None),
        (["argo", "version"], "version"),
        (["argo", "--tui", "chat"], "chat"),
        (["argo", "-w", "logs"], "logs"),
        (["argo", "chat", "hello world"], "chat"),
        (["argo", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["argo", "-m", "gpt5", "chat"], "chat"),
        (["argo", "--model", "gpt5", "chat", "hi"], "chat"),
        (["argo", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["argo", "-z", "hello world"], None),
        (["argo", "-z", "hello", "chat"], "chat"),
        (["argo", "--model=gpt5", "chat"], "chat"),     # inline form
        (["argo", "--", "chat"], "chat"),               # -- terminator
        (["argo", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["argo", "some-plugin-cmd"], "some-plugin-cmd"),
        (["argo", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["argo"],                          # bare → chat
        ["argo", "--help"],                # top-level help
        ["argo", "-h"],
        ["argo", "version"],               # known built-in
        ["argo", "logs"],
        ["argo", "gateway", "run"],
        ["argo", "--tui"],
        ["argo", "-w", "--tui"],
        ["argo", "chat", "hi"],
        ["argo", "help"],                  # accepted built-in-ish
        ["argo", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["argo", "meet", "join"],          # potential google_meet plugin
        ["argo", "honcho", "status"],      # potential memory plugin
        ["argo", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``argo help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"argo_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
