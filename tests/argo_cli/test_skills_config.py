"""Tests for argo_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("argo_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from argo_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("argo_cli.config.load_config")
    ...
    @patch("argo_cli.config.load_config")
    @patch.dict("os.environ", {"ARGO_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("ARGO_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """ARGO_SESSION_PLATFORM should be used when ARGO_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_argo_platform_takes_precedence(self, tmp_path, monkeypatch):
        """ARGO_PLATFORM should win over ARGO_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("ARGO_HOME", str(tmp_path))
        monkeypatch.setenv("ARGO_PLATFORM", "telegram")
        monkeypatch.setenv("ARGO_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ARGO_HOME", str(tmp_path))
        monkeypatch.setenv("ARGO_PLATFORM", "telegram")
        monkeypatch.setenv("ARGO_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from argo_cli.skills_config import _get_categories
        skills = [
