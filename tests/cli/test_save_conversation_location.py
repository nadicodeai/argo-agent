...
Regression: the old implementation wrote ``argo_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``argo sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.argo/sessions/saved/`` and prints
...
def argo_home(tmp_path, monkeypatch):
    ...
    # Clear any cached argo_home computation
    import argo_constants
    if hasattr(argo_constants, "_argo_home_cache"):
        argo_constants._argo_home_cache = None
    return home
...
def test_save_conversation_writes_under_argo_home(argo_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.argo/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the ARGO_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "argo_constants"]:
    ...
    cli.ArgoCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("argo_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.argo/sessions/saved/
    saved_dir = argo_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("argo_conversation_*.json"))
    ...
    assert "argo --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(argo_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "argo_constants"]:
        sys.modules.pop(mod, None)
    import cli
