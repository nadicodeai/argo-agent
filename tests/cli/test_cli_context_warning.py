...
def _isolate(tmp_path, monkeypatch):
    """Isolate ARGO_HOME so tests don't touch real config."""
    home = tmp_path / ".argo"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ArgoCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ArgoCLI.__new__(ArgoCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["argo-core"]
        obj.compact = False
