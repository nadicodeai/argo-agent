...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("argo/argo-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned argo/* and pr-* branches."""

    def test_prunes_orphaned_argo_branch(self, git_repo):
        """argo/argo-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "argo/argo-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "argo/argo-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "argo/argo-deadbeef" in result.stdout
        ...
        assert "argo/argo-deadbeef" in orphaned
        ...
        assert "argo/argo-deadbeef" not in result.stdout
