"""Tests for argo_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Argo entry point
(argo, argo-agent, argo-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Argo entry point imports argo_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported argo_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("argo_bootstrap", None)
    import argo_bootstrap  # noqa: WPS433
    return argo_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Argo CLI
...
class TestEntryPointsImportBootstrap:
    """Every Argo entry point must import argo_bootstrap as its
    ...
    # Entry points that invoke Argo as a process.  Each one must
    # import argo_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "argo_cli/main.py",   # argo CLI (console_script)
        "run_agent.py",          # argo-agent (console_script)
        "acp_adapter/entry.py",  # argo-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import argo_bootstrap' and that
        ...
        half-finished ``argo update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``argo_bootstrap`` as a top-level module) leaves argo
        ...
        """
        # Resolve relative to the argo-agent repo root.  Tests live
        # at tests/test_argo_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # argo start even when argo_bootstrap hasn't been
        ...
        assert first_import_name == "argo_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'argo_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import argo_bootstrap' line to be the first import."
        )
