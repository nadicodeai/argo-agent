...
substrings like 'foo@argo_bot.example'.
...
def _make_adapter():
    ...
    adapter._bot = SimpleNamespace(id=999, username="argo_bot")
    return adapter


def _mention_entity(text, mention="@argo_bot"):
...
class TestRealMentionsAreDetected:
    ...
    def test_mention_mid_sentence(self):
        adapter = _make_adapter()
        text = "hey @argo_bot, can you help?"
        msg = _message(text=text, entities=[_mention_entity(text)])
        assert adapter._message_mentions_bot(msg) is True

    def test_mention_at_end_of_message(self):
        adapter = _make_adapter()
        text = "thanks for looking @argo_bot"
        msg = _message(text=text, entities=[_mention_entity(text)])
        assert adapter._message_mentions_bot(msg) is True

    def test_mention_in_caption(self):
        adapter = _make_adapter()
        caption = "photo for @argo_bot"
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@argo_bot.example'."""
        adapter = _make_adapter()
        msg = _message(text="email me at foo@argo_bot.example")
        assert adapter._message_mentions_bot(msg) is False

    def test_hostname_substring(self):
        adapter = _make_adapter()
        msg = _message(text="contact user@argo_bot.domain.com")
        assert adapter._message_mentions_bot(msg) is False

    def test_superstring_username(self):
        """`@argo_botx` is a different username; Telegram would emit a mention
        entity for `@argo_botx`, not `@argo_bot`."""
        adapter = _make_adapter()
        msg = _message(text="@argo_botx hello")
        assert adapter._message_mentions_bot(msg) is False

    def test_underscore_suffix_substring(self):
        adapter = _make_adapter()
        msg = _message(text="see @argo_bot_admin for help")
        assert adapter._message_mentions_bot(msg) is False
    ...
    def test_substring_inside_code_block_without_entity(self):
        ...
        adapter = _make_adapter()
        msg = _message(text="use the string `@argo_bot` in config")
        assert adapter._message_mentions_bot(msg) is False
    ...
    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@argo_bot.example")
        assert adapter._message_mentions_bot(msg) is False
...
class TestCaseInsensitivity:
    ...
    def test_uppercase_mention(self):
        adapter = _make_adapter()
        text = "hi @ARGO_BOT"
        msg = _message(text=text, entities=[_mention_entity(text, mention="@ARGO_BOT")])
        assert adapter._message_mentions_bot(msg) is True
