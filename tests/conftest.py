"""Shared fixtures for the argo-agent test suite.
...
2. **Isolated ARGO_HOME.** ARGO_HOME points to a per-test tempdir so
   code reading ``~/.argo/*`` via ``get_argo_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".argo"`` instead of the canonical
   ``get_argo_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ARGO_SESSION_* inheritance** — the agent's current gateway
...
# ARGO_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ARGO_BEHAVIORAL_VARS = frozenset({
    "ARGO_YOLO_MODE",
    "ARGO_INTERACTIVE",
    "ARGO_QUIET",
    "ARGO_TOOL_PROGRESS",
    ...
    "ARGO_SESSION_CHAT_ID",
    "ARGO_SESSION_CHAT_NAME",
    "ARGO_SESSION_THREAD_ID",
    "ARGO_SESSION_SOURCE",
    "ARGO_SESSION_KEY",
    "ARGO_GATEWAY_SESSION",
    "ARGO_PLATFORM",
    "ARGO_MODEL",
    "ARGO_INFERENCE_MODEL",
    ...
    "ARGO_TIMEZONE",
    "ARGO_REDACT_SECRETS",
    "ARGO_BACKGROUND_NOTIFICATIONS",
    "ARGO_EXEC_ASK",
    "ARGO_HOME_MODE",
    "ARGO_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.argo/kanban.db instead of the per-test ARGO_HOME.
    "ARGO_KANBAN_DB",
    "ARGO_KANBAN_BOARD",
    "ARGO_KANBAN_HOME",
    "ARGO_KANBAN_WORKSPACES_ROOT",
    "ARGO_KANBAN_LOGS_ROOT",
    "ARGO_KANBAN_TASK",
    "ARGO_KANBAN_WORKSPACE",
    "ARGO_KANBAN_RUN_ID",
    "ARGO_KANBAN_CLAIM_LOCK",
    "ARGO_KANBAN_DISPATCH_IN_GATEWAY",
    "ARGO_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ARGO_HOME to per-test tempdirs so code that
    reads ``~/.argo/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ARGO_* vars that could change test semantics.
    for name in _ARGO_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ARGO_HOME to a per-test tempdir. Code that reads
    #    ``~/.argo/*`` via ``get_argo_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.argo/*`` via
    #    ``Path.home() / ".argo"`` instead of ``get_argo_home()``
    #    is a bug to fix at the callsite.
    fake_argo_home = tmp_path / "argo_test"
    fake_argo_home.mkdir()
    (fake_argo_home / "sessions").mkdir()
    (fake_argo_home / "cron").mkdir()
    (fake_argo_home / "memories").mkdir()
    (fake_argo_home / "skills").mkdir()
    monkeypatch.setenv("ARGO_HOME", str(fake_argo_home))
    ...
    #    ~/.argo/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import argo_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_argo_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal argo config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``argo-gateway`` process
...
# correlated with a ``tests/argo_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> argo-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart argo-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting argo/python patterns are also blocked.
    """
    ...
    def _matches_argo_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ARGO_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_argo_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch argo-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m argo_cli.main".
                if (
                    "argo" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live argo-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting argo/python could hit the live gateway. "
