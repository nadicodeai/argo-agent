...
def server():
    with patch.dict(
        "sys.modules",
        {
            "argo_constants": MagicMock(
                get_argo_home=MagicMock(return_value="/tmp/argo_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'argo-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'argo-release' patched"
    }
