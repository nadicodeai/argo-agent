...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"argo-agent[termux]"' in text
    assert '"argo-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"argo-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
