"""Tests for argo_constants module."""
...
class TestGetDefaultArgoRoot:
    """Tests for get_default_argo_root() — Docker/custom deployment awareness."""

    def test_no_argo_home_returns_native(self, tmp_path, monkeypatch):
        """When ARGO_HOME is not set, returns ~/.argo."""
        monkeypatch.delenv("ARGO_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_argo_root() == tmp_path / ".argo"

    def test_argo_home_is_native(self, tmp_path, monkeypatch):
        """When ARGO_HOME = ~/.argo, returns ~/.argo."""
        native = tmp_path / ".argo"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARGO_HOME", str(native))
        assert get_default_argo_root() == native

    def test_argo_home_is_profile(self, tmp_path, monkeypatch):
        """When ARGO_HOME is a profile under ~/.argo, returns ~/.argo."""
        native = tmp_path / ".argo"
    ...
    def test_argo_home_is_docker(self, tmp_path, monkeypatch):
        """When ARGO_HOME points outside ~/.argo (Docker), returns ARGO_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARGO_HOME", str(docker_home))
        assert get_default_argo_root() == docker_home

    def test_argo_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any ARGO_HOME outside ~/.argo is treated as the root."""
        custom = tmp_path / "my-argo-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARGO_HOME", str(custom))
        assert get_default_argo_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (ARGO_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ARGO_HOME", str(profile))
        assert get_default_argo_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(argo_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(argo_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".argo"
