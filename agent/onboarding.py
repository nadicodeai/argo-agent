...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Argo starts and finds ``~/.openclaw/``.

    Points users at ``argo claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``argo claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Argo, run "
        "`argo claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `argo claw cleanup` (renames it to ~/.openclaw.pre-migration — "
