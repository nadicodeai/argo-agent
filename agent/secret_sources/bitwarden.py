...
Argo pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.argo/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<argo_home>/bin/bws`` on
  first use.  Argo pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.argo/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``argo`` invocations don't
  hammer the API.
* Failures NEVER block Argo startup.  Missing binary, no network,
...
# In-process cache so repeated load_argo_dotenv() calls (CLI startup,
...
def _argo_bin_dir() -> Path:
    """Where Argo stores its managed binaries.  Profile-aware."""
    from argo_constants import get_argo_home

    return get_argo_home() / "bin"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<argo_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _argo_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``argo secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _argo_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="argo-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "argo-agent"})
    try:
...
# ---------------------------------------------------------------------------
# Public entry point — called from argo_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_argo_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `argo secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `argo secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `argo secrets bitwarden setup` to install."
        )
        return result
