...
Every Argo request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Argo Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=argo-agent",
        "client=argo-client-v<__version__>",
    ]

The version is sourced live from ``argo_cli.__version__`` so it auto-aligns
...
``argo_cli.__version__`` is the canonical source of truth.
"""
...
def _argo_version() -> str:
    """Return the current Argo release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``argo_cli`` cannot be imported (should
...
def argo_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=argo-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=argo-client-v{_argo_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=argo-agent", argo_client_tag()]
