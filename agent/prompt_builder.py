...
from argo_constants import get_argo_home, get_skills_dir, is_wsl
...
_ARGO_MD_NAMES = (".argo.md", "ARGO.md")


def _find_argo_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.argo.md`` or ``ARGO.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Argo Agent, an intelligent AI assistant created by Nous Research. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.argo/kanban.db`. Your task id is in "
    "`$ARGO_KANBAN_TASK`; your workspace is `$ARGO_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ARGO_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `argo kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Argo WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Argo itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Argo runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Argo itself is running. The host OS, home, and cwd "
                f"of the Argo process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Argo "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_argo_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.argo/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("ARGO_PLATFORM")
        or get_session_env("ARGO_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Argo Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `argo-agent` skill "
            "first. It has the actual commands (e.g. `argo config set …`, `argo tools`, "
            "`argo setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: argo setup, argo setup tools, argo setup terminal, argo status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from ARGO_HOME and return its content, or None.
    ...
    """
    try:
        from argo_cli.config import ensure_argo_home
        ensure_argo_home()
    except Exception as e:
        logger.debug("Could not ensure ARGO_HOME before loading SOUL.md: %s", e)

    soul_path = get_argo_home() / "SOUL.md"
...
def _load_argo_md(cwd_path: Path) -> str:
    """.argo.md / ARGO.md — walk to git root."""
    argo_md_path = _find_argo_md(cwd_path)
    if not argo_md_path:
        return ""
    try:
        content = argo_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = argo_md_path.name
        try:
            rel = str(argo_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".argo.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", argo_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .argo.md / ARGO.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from ARGO_HOME is independent and always included when present.
    ...
    # SOUL.md from ARGO_HOME only — skip when already loaded as identity
    if not skip_soul:
