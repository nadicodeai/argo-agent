"""Projects codex app-server events into Argo' messages list.

The translator that lets Argo' memory/skill review keep working under the
...
Argo' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Argo
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Argo' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Argo stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Argo already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Argo' messages store text only.
        text_parts: list[str] = []
