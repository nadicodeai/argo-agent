...
Owns one Codex thread per Argo session. Drives `turn/start`, consumes
...
# Argo' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_ARGO_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Argo session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _ARGO_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("ARGO_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="argo",
            client_title="Argo Agent",
            client_version=_get_argo_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Argo' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Argo' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own argo-tools callback we
            # auto-accept — the user already approved Argo' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Argo approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Argo returns 'once', 'session', 'always', or 'deny'.
...
def _get_argo_version() -> str:
    """Best-effort Argo version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("argo-agent")
