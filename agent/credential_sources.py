"""Unified removal contract for every credential source Argo reads from.

Argo seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.argo/.env
    claude_code   — ~/.claude/.credentials.json
    argo_pkce   — ~/.argo/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``argo auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), argo_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.argo/.env  → clear it
    ...
    """
    from argo_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Argo.",
            f"  The pool entry is now suppressed — Argo will ignore "
            f"{env_var} until you run `argo auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Argo stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `argo auth add anthropic` to re-enable if needed.",
    ])


def _remove_argo_pkce(provider: str, removed) -> RemovalResult:
    """~/.argo/.anthropic_oauth.json is ours — delete it outright."""
    from argo_constants import get_argo_home

    result = RemovalResult()
    oauth_file = get_argo_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Argo Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `argo login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `argo auth add nous` to re-engage, which is the documented re-add
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``argo auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``argo auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `argo model` → xAI Grok OAuth (SuperGrok Subscription) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Argo auth store is not enough — _seed_from_singletons() would
    ...
    via ``argo auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `argo auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `argo auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Argo stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `argo auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="argo_pkce",
        remove_fn=_remove_argo_pkce,
        description="~/.argo/.anthropic_oauth.json",
    ))
    register(RemovalStep(
