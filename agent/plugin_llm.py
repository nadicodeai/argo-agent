...
Plugins built on Argo Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Argo
...
:class:`~argo_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Argo' main config
...
handles every provider, fallback chain, and per-task override Argo
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`argo_cli.plugins.PluginContext`
