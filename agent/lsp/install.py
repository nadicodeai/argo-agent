...
appropriate.  All installs go to a Argo-owned bin staging dir,
``<ARGO_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``argo lsp
  status``.
...
# ``<ARGO_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def argo_lsp_bin_dir() -> Path:
    """Return the Argo-owned bin staging dir for LSP servers."""
    home = os.environ.get("ARGO_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".argo")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = argo_lsp_bin_dir().parent  # <ARGO_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a argo-owned target dir.
    ...
    console script.
    """
    pip_target = argo_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = argo_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``argo lsp status`` CLI to give users a quick
