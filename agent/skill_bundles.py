...
Storage
-------
Bundles live in ``~/.argo/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ARGO_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``argo bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ARGO_HOME.

    Honors ``ARGO_BUNDLES_DIR`` for tests; falls back to
    ``<ARGO_HOME>/skill-bundles``.
    """
    override = os.environ.get("ARGO_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_argo_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `argo bundles` CLI subcommand.
# ---------------------------------------------------------------------------
