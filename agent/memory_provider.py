...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - argo_home (str): The active ARGO_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.argo``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "argo").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'argo memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], argo_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'argo memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``argo_home`` is the active ARGO_HOME directory path.
