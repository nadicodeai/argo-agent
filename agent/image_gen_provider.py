...
as ``kind: backend``) or ``~/.argo/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$ARGO_HOME/cache/images/``, creating parents as needed."""
    from argo_constants import get_argo_home

    path = get_argo_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ARGO_HOME/cache/images/``.
