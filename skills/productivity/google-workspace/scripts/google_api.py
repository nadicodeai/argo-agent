#!/usr/bin/env python3
"""Google Workspace API CLI for Argo Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Argo-facing JSON contract and falls back to the Python client
...
from _argo_home import get_argo_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ARGO_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Argo Agent")
