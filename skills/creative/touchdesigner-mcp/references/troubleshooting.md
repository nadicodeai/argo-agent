...
## 5. Argo Configuration

### Config location

`$ARGO_HOME/config.yaml` (defaults to `~/.argo/config.yaml` when `ARGO_HOME` is unset)
...
Restart the Argo session for changes to take effect. The MCP connection is
