...
ARGO_HOME_DIR="${ARGO_HOME:-$HOME/.argo}"
ARGO_CFG="${ARGO_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Argo config has twozero_td MCP entry ──
if [[ ! -f "$ARGO_CFG" ]]; then
    echo -e " ${FAIL} Argo config not found at ${ARGO_CFG}"
    manual_steps+=("Create ${ARGO_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ARGO_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Argo config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Argo config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ARGO_CFG} manually"); }
    manual_steps+=("Restart Argo session to pick up config change")
fi
