...
## 5. GODMODE FAST — Argo 4 405B (Zero Refusal)

**Model:** `nousresearch/argo-4-405b`
**Strategy:** Argo models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Argo Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.argo/config.yaml`:
