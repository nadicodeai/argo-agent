...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ARGO_HOME", os.path.expanduser("~/.argo")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ARGO_HOME", Path.home() / ".argo")) / "skills" / "red-teaming" / "godmode"
...
ARGO_HOME = Path(os.getenv("ARGO_HOME", Path.home() / ".argo"))
CONFIG_PATH = ARGO_HOME / "config.yaml"
PREFILL_PATH = ARGO_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Argo models — already uncensored, just needs clean prompt
    "argo": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "argo" in model_lower or "nous" in model_lower:
        return "argo"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Argo config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.argo/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Argo for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Argo for changes to take effect.")
