# nix/nixosModules.nix — NixOS module for argo-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: argo runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ARGO_HOME/.env
# and read by argo at startup — no container recreation needed for env changes.
#
# Tool resolution: the argo wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.argo-agent;
    effectivePackage =
    ...
    argo-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-argo-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "argo-config-attrs";
      description = "Argo YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "argo-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "argo-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ARGO_DOC_EOF'\n${value}\nARGO_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "argo-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/argo";
    ...
    # argo user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "argo-container-entrypoint" ''
      set -eu

      ARGO_UID="''${ARGO_UID:?ARGO_UID must be set}"
      ARGO_GID="''${ARGO_GID:?ARGO_GID must be set}"

      # ── Group: ensure a group with GID=$ARGO_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ARGO_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="argo"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ARGO_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ARGO_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ARGO_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ARGO_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="argo"
        TARGET_HOME="/home/argo"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ARGO_UID" -g "$ARGO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ARGO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ARGO_UID:$ARGO_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ARGO_HOME is owned by the target user.
      ...
      if [ -n "''${ARGO_HOME:-}" ] && [ -d "$ARGO_HOME" ]; then
        find "$ARGO_HOME" \! -user "$ARGO_UID" -exec chown "$ARGO_UID:$ARGO_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/argo-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/argo-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/argo ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/argo
        chmod 0440 /etc/sudoers.d/argo
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ARGO_UID" --regid="$ARGO_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ARGO_HOME/.env.
    ...
    # Default: /var/lib/argo/workspace → /data/workspace.
  ...
  in {
    options.services.argo-agent = with lib; {
      enable = mkEnableOption "Argo Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = argo-agent;
        description = "The argo-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/argo";
        description = "State directory. Contains .argo/ subdir (ARGO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Argo config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ARGO_HOME/.env at activation time.
          Argo reads this file on every startup via load_argo_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ARGO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ARGO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `argo gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the argo user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the argo plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Argo discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          argo_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as argo (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in argo-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the argo CLI to environment.systemPackages and export
          ARGO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.argo symlink to the service
            stateDir. These users are automatically added to the argo group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.argo-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the argo CLI to system PATH and export ARGO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.argo/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ARGO_HOME = "${cfg.stateDir}/.argo";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.argo-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the argo user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.argo-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed argo
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure argo is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.argo        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.argo/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.argo/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.argo/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.argo/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.argo/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."argo-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.argo
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.argo ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.argo ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.argo -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.argo/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.argo/$_subdir"
            chmod 2770 "${cfg.stateDir}/.argo/$_subdir"
            find "${cfg.stateDir}/.argo/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.argo/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.argo/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.argo/config.yaml
            chmod 0640 ${cfg.stateDir}/.argo/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.argo/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.argo/.managed
          chmod 0644 ${cfg.stateDir}/.argo/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.argo/.container-mode <<'ARGO_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    argo_bin=${containerDataDir}/current-package/bin/argo
    ARGO_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.argo/.container-mode
            chmod 0644 ${cfg.stateDir}/.argo/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.argo/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.argo";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.argo" ]; then
                  rm -f "${symlinkPath}"
                  echo "argo-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.argo -> stateDir/.argo for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.argo";
                target = "${cfg.stateDir}/.argo";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "argo-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.argo/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.argo/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.argo/auth.json
              fi
            ''}
          ''}
          ...
          # Argo reads $ARGO_HOME/.env at startup via load_argo_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.argo/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ARGO_NIX_ENV_EOF'
    ${envFileContent}
    ARGO_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.argo/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.argo/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.argo/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.argo-agent = {
          description = "Argo Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $ARGO_HOME/.env by the activation script. load_argo_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/argo"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the argo group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.argo-agent = {
          description = "Argo Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ARGO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ARGO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ARGO_UID="$ARGO_UID" \
                --env ARGO_GID="$ARGO_GID" \
                --env ARGO_HOME=${containerDataDir}/.argo \
                --env ARGO_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/argo gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
