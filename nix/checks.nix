...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      argo-agent = self'.packages.default;
      argoVenv = argo-agent.argoVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "argo-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${argoVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "argo-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "argo-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${argo-agent}/bin/argo || (echo "FAIL: argo binary missing"; exit 1)
          test -x ${argo-agent}/bin/argo-agent || (echo "FAIL: argo-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${argo-agent}/bin/argo version 2>&1 | grep -qi "argo" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "argo-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in argo argo-agent argo-acp; do
            test -x ${argo-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "argo-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking argo --help ==="
          ${argo-agent}/bin/argo --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${argo-agent}/bin/argo --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "argo-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${argo-agent}/share/argo-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${argo-agent}/share/argo-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ARGO_BUNDLED_SKILLS" ${argo-agent}/bin/argo || \
            (echo "FAIL: ARGO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ARGO_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "argo-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${argo-agent}/share/argo-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${argo-agent}/share/argo-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ARGO_BUNDLED_PLUGINS" ${argo-agent}/bin/argo || \
            (echo "FAIL: ARGO_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ARGO_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "argo-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${argo-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${argo-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ARGO_TUI_DIR" ${argo-agent}/bin/argo || \
            (echo "FAIL: ARGO_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ARGO_TUI_DIR set in wrapper"
        ...
        # Verify ARGO_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        argo-node = pkgs.runCommand "argo-node-version" { } ''
          set -e
          echo "=== Checking ARGO_NODE in wrapper ==="
          grep -q "ARGO_NODE" ${argo-agent}/bin/argo || \
            (echo "FAIL: ARGO_NODE not set in wrapper"; exit 1)
          echo "PASS: ARGO_NODE present in wrapper"

          ARGO_NODE=$(sed -n "s/^export ARGO_NODE='\(.*\)'/\1/p" ${argo-agent}/bin/argo)
          test -x "$ARGO_NODE" || (echo "FAIL: ARGO_NODE=$ARGO_NODE not executable"; exit 1)
          echo "PASS: ARGO_NODE executable at $ARGO_NODE"

          NODE_MAJOR=$("$ARGO_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ARGO_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ARGO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "argo-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ARGO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ARGO_MANAGED guards ==="
          check_blocked "config set" ${argo-agent}/bin/argo config set model foo
          check_blocked "config edit" ${argo-agent}/bin/argo config edit
        ...
        extra-python-packages = let
          ...
          argoWithExtra = argo-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "argo-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${argoWithExtra}/bin/argo || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${argoWithExtra}/bin/argo || \
          ...
          if grep -q "PYTHONPATH" ${argo-agent}/bin/argo; then
        ...
        extra-dependency-groups = let
          argoWithGroups = argo-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "argo-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${argoWithGroups}"
          echo "venv: ${argoWithGroups.argoVenv}"
        ...
        in pkgs.runCommand "argo-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            ${configMergeScript} ${nixSettings} "$argo_home/config.yaml"
            ${argoVenv}/bin/python3 -c '
import json, sys
