...
# 5. Symlinks the 'argo' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Argo Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/argo-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink argo into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up argo command..."

ARGO_BIN="$SCRIPT_DIR/venv/bin/argo"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ARGO_BIN" "$COMMAND_LINK_DIR/argo"
echo -e "${GREEN}✓${NC} Symlinked argo → $COMMAND_LINK_DISPLAY_DIR/argo"
...
# ============================================================================
# Seed bundled skills into ~/.argo/skills/
# ============================================================================

ARGO_SKILLS_DIR="${ARGO_HOME:-$HOME/.argo}/skills"
mkdir -p "$ARGO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.argo/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ARGO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  argo status        # Check configuration"
if is_termux; then
    echo "  argo gateway       # Run gateway in foreground"
else
    echo "  argo gateway install # Install gateway service (messaging + cron)"
fi
echo "  argo cron list     # View scheduled jobs"
echo "  argo doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m argo_cli.main setup
fi
