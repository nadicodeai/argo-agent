...
# Python-driven tool session (e.g. Argo terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/argo-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/argo-agent.git"
ARGO_HOME="${ARGO_HOME:-$HOME/.argo}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ARGO_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ARGO_INSTALL_DIR"
...
#   code at /usr/local/lib/argo-agent, command at /usr/local/bin/argo,
#   data still at /root/.argo (ARGO_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.argo/argo-agent"
            echo "                   default (root, Linux): /usr/local/lib/argo-agent"
            echo "  --argo-home PATH  Data directory (default: ~/.argo, or \$ARGO_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Argo installs the code under"
            echo "  /usr/local/lib/argo-agent and links the command into"
            echo "  /usr/local/bin/argo (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ARGO_HOME"
            echo "  (default /root/.argo).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ARGO_HOME/argo-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs argo setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `argo` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ARGO_HOME/argo-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ARGO_HOME/argo-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/argo-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ARGO_HOME/argo-agent — then preserve it)
#
# Always no-op when the user set --dir or $ARGO_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ARGO_HOME.
    if is_termux; then
        INSTALL_DIR="$ARGO_HOME/argo-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ARGO_HOME/argo-agent/.git" ]; then
            INSTALL_DIR="$ARGO_HOME/argo-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/argo-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/argo-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/argo"
        log_info "  Data:    $ARGO_HOME (unchanged)"
        return 0
    fi
...
get_argo_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/argo" ]; then
        echo "$link_dir/argo"
    else
        echo "argo"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/NousResearch/argo-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/argo-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/argo-uv-installer.$$.sh")"
...
check_node() {
    ...
    if [ -x "$ARGO_HOME/node/bin/node" ]; then
        export PATH="$ARGO_HOME/node/bin:$PATH"
        local found_ver=$("$ARGO_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Argo-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.argo/node/..."
    ...
    # Place into ~/.argo/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$ARGO_HOME/node"
    mkdir -p "$ARGO_HOME"
    mv "$extracted_dir" "$ARGO_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$ARGO_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$ARGO_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$ARGO_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$ARGO_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ARGO_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.argo/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Argo install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Argo Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="argo-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Argo behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up argo command..."

    if [ "$USE_VENV" = true ]; then
        ARGO_BIN="$INSTALL_DIR/venv/bin/argo"
    else
        ARGO_BIN="$(which argo 2>/dev/null || echo "")"
        if [ -z "$ARGO_BIN" ]; then
            log_warn "argo not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ARGO_BIN" ]; then
        log_warn "argo entry point not found at $ARGO_BIN"
    ...
    # Create a user-facing shim for the argo command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $ARGO_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$ARGO_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/argo"
    cat > "$command_link_dir/argo" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$ARGO_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/argo"
    log_success "Installed argo launcher → $command_link_display_dir/argo"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "argo command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v argo' \
                >/dev/null 2>&1; then
        ...
        log_info "argo not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Argo Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Argo Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Argo Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so argo works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.argo directory structure (config at top level, code in subdir)
    mkdir -p "$ARGO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.argo/.env (top level, easy to find)
    if [ ! -f "$ARGO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ARGO_HOME/.env"
            log_success "Created ~/.argo/.env from template"
        else
            touch "$ARGO_HOME/.env"
            log_success "Created ~/.argo/.env"
        fi
    else
        log_info "~/.argo/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$ARGO_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.argo/config.yaml (top level, easy to find)
    if [ ! -f "$ARGO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ARGO_HOME/config.yaml"
            log_success "Created ~/.argo/config.yaml from template"
        fi
    else
        log_info "~/.argo/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ARGO_HOME/SOUL.md" ]; then
        cat > "$ARGO_HOME/SOUL.md" << 'SOUL_EOF'
# Argo Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Argo communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.argo/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.argo/"

    # Seed bundled skills into ~/.argo/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.argo/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.argo/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ARGO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$ARGO_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.argo/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Argo Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Argo will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (argo --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'argo setup' after install."
        return 0
    fi
    ...
    # Run argo setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m argo_cli.main setup < /dev/tty
    else
        python -m argo_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ARGO_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Argo to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ARGO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'argo whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ARGO_CMD="$(get_argo_command_path)"
                $ARGO_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'argo whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'argo gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ARGO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ARGO_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: argo gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: argo gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ARGO_CMD gateway > "$ARGO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.argo/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: argo gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: argo gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ARGO_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ARGO_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ARGO_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}argo${NC}              Start chatting"
    echo -e "   ${GREEN}argo setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}argo config${NC}       View/edit configuration"
    echo -e "   ${GREEN}argo config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}argo gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}argo update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'argo' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'argo' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'argo' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$ARGO_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$ARGO_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$ARGO_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$ARGO_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$ARGO_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Argo for pip install"
    ...
    if [ -n "$ARGO_CMD" ]; then
        log_info "Running argo setup..."
        "$ARGO_CMD" setup
    else
        log_warn "argo command not found on PATH"
        log_info "Try: python -m argo_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$ARGO_HOME/.install_method"
}
