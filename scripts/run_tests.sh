#!/usr/bin/env bash
# Canonical test runner for argo-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.argo/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.argo/argo-agent/venv"; do
...
if [ -f "$HOME/.argo/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.argo"
