...
#   2. ~/.argo/node/ from a prior Argo-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.argo/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$ARGO_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   ARGO_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   ARGO_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   ARGO_HOME               (default: $HOME/.argo)
# ============================================================================

ARGO_NODE_MIN_VERSION="${ARGO_NODE_MIN_VERSION:-20}"
ARGO_NODE_TARGET_MAJOR="${ARGO_NODE_TARGET_MAJOR:-22}"
ARGO_HOME="${ARGO_HOME:-$HOME/.argo}"
ARGO_NODE_AVAILABLE=false
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$ARGO_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $ARGO_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$ARGO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$ARGO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $ARGO_NODE_TARGET_MAJOR..."
    proto install node "$ARGO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $ARGO_NODE_TARGET_MAJOR..."
    nvm install "$ARGO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$ARGO_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${ARGO_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${ARGO_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${ARGO_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $ARGO_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $ARGO_HOME/node/..."
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $ARGO_HOME/node/"
    return 0
}
...
ensure_node() {
    ...
    if [ -x "$ARGO_HOME/node/bin/node" ]; then
        export PATH="$ARGO_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Argo-managed)"
            ARGO_NODE_AVAILABLE=true
            return 0
        fi
    fi
    ...
    _nb_try_fnm   && { ARGO_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { ARGO_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { ARGO_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { ARGO_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { ARGO_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { ARGO_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $ARGO_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
