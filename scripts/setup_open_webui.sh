...
# Bootstrap Open WebUI against Argo Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.argo/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-argo.sh
...
#   ARGO_API_PORT=8642
#   ARGO_API_HOST=127.0.0.1
#   ARGO_API_MODEL_NAME='Argo Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Argo Agent WebUI}"
...
ARGO_ENV_FILE="${ARGO_ENV_FILE:-$HOME/.argo/.env}"
ARGO_API_PORT="${ARGO_API_PORT:-8642}"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$ARGO_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$ARGO_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.argo.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.argo</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.argo"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.argo"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-argo.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Argo Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-argo.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.argo/logs/openwebui.log
StandardError=append:%h/.argo/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-argo.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$ARGO_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Argo API server is configured...'
  upsert_env API_SERVER_ENABLED true "$ARGO_ENV_FILE"
  upsert_env API_SERVER_HOST "$ARGO_API_HOST" "$ARGO_ENV_FILE"
  upsert_env API_SERVER_PORT "$ARGO_API_PORT" "$ARGO_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$ARGO_API_MODEL_NAME" "$ARGO_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$ARGO_ENV_FILE"
  ensure_env_permissions
  ...
  log "Argo API endpoint: ${ARGO_API_BASE_URL}"
