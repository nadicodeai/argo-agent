#!/usr/bin/env python3
"""Build the Argo Model Catalog — a centralized JSON manifest of curated models.
...
Argo CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Argo release.
...
# Ensure ARGO_HOME is set for imports that touch it at module level.
os.environ.setdefault("ARGO_HOME", os.path.join(os.path.expanduser("~"), ".argo"))

from argo_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS  # noqa: E402
...
def build_catalog() -> dict:
    return {
        ...
        "metadata": {
            "source": "argo-agent repo",
            "docs": "https://argo-agent.nousresearch.com/docs/reference/model-catalog",
        },
        "providers": {
            "openrouter": {
                "metadata": {
