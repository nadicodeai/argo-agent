...
VERSION_FILE = REPO_ROOT / "argo_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+argo-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "argo@nousresearch.com": "NousResearch",
    ...
    "argo@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (ARGO_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (ARGO_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: argo-eval community link)
    ...
    "argoagent26@gmail.com": "argoagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+argoagent26@users.noreply.github.com": "argoagent26",
    ...
    "vigo@argo": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"argo-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "argo@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/argo-agent",
    ...
    lines.append(f"# Argo Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Argo Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Argo Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Argo Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Argo Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Argo Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Argo Agent v{new_version} ({calver_date})' "
