# ============================================================================
# Argo Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$ArgoHome = "$env:LOCALAPPDATA\argo",
    [string]$InstallDir = "$env:LOCALAPPDATA\argo\argo-agent",
...
$RepoUrlSsh = "git@github.com:NousResearch/argo-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/argo-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Argo Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ArgoHome "node"
...
function Install-Git {
    ...
    which Argo uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\argo\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Argo needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Argo copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\argo\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ARGO_GIT_BASH_PATH`` (User scope) so Argo can find it in a fresh
    ...
    # Download PortableGit into $ArgoHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ArgoHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Argo features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Argo needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ARGO_GIT_BASH_PATH`` (User env scope) so Argo can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ArgoHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ArgoHome\git\usr\bin\bash.exe
    $candidates += "$ArgoHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ArgoHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ARGO_GIT_BASH_PATH", $candidate, "User")
            $env:ARGO_GIT_BASH_PATH = $candidate
            Write-Info "Set ARGO_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Argo may not find Git Bash."
    Write-Info "If needed, set ARGO_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$ArgoHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$ArgoHome\node;$env:Path"
        Write-Success "Node.js $version found (Argo-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $ArgoHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ArgoHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\argo-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ArgoHome\node") { Remove-Item -Recurse -Force "$ArgoHome\node" }
                Move-Item $extractedDir.FullName "$ArgoHome\node"
                ...
                $version = & "$ArgoHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ArgoHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running argo processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/argo-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/argo-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/argo-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\argo-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\argo-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `argo.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install argo-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and argo.exe broken with
    ...
    # here, not 30 seconds later when the user runs `argo`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `argo dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `argo dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `argo dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up argo command..."
    
    if ($NoVenv) {
        $argoBin = "$InstallDir"
    } else {
        $argoBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so argo is globally available
    # On Windows, the argo.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$argoBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$argoBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $argoBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ARGO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\argo instead
    # of the Unix default ~/.argo
    $currentArgoHome = [Environment]::GetEnvironmentVariable("ARGO_HOME", "User")
    if (-not $currentArgoHome -or $currentArgoHome -ne $ArgoHome) {
        [Environment]::SetEnvironmentVariable("ARGO_HOME", $ArgoHome, "User")
        Write-Success "Set ARGO_HOME=$ArgoHome"
    }
    $env:ARGO_HOME = $ArgoHome
    
    # Update current session
    $env:Path = "$argoBin;$env:Path"
    
    Write-Success "argo command ready"
}
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$ArgoHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgoHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgoHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgoHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgoHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgoHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgoHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgoHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ArgoHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ArgoHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.argo/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.argo/.env"
        }
    } else {
        Write-Info "~/.argo/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ArgoHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.argo/config.yaml from template"
        }
    } else {
        Write-Info "~/.argo/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Argo's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.argo/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.argo/"
    
    # Seed bundled skills into ~/.argo/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.argo/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.argo/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ArgoHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.argo/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'argo setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\argo-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\argo-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\argo-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.argo/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'argo setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m argo_cli.main setup
    } else {
        python -m argo_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $argoCmd = "$InstallDir\venv\Scripts\argo.exe"
    if (-not (Test-Path $argoCmd)) {
        $argoCmd = "argo"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ArgoHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'argo whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: argo gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ArgoHome\logs\gateway.log"
            Start-Process -FilePath $argoCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ArgoHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: argo gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: argo gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ArgoHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArgoHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArgoHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ArgoHome\argo-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   argo              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   argo setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   argo config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   argo config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   argo gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   argo update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Argo repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Argo to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/argo-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
