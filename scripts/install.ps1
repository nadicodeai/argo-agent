...
$RepoUrlSsh = "git@github.com:nadicodeai/argo-agent.git"
$RepoUrlHttps = "https://github.com/nadicodeai/argo-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/nadicodeai/argo-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/nadicodeai/argo-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/nadicodeai/argo-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/nadicodeai/argo-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
