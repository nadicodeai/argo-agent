...
## Token Lifecycle
- Store at `~/.argo/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `argo_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `argo_cli/models.py` — Add Gemini model catalog
4. `argo_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `argo_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `argo_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- OR: accept user-provided client_id via ARGO_GEMINI_CLIENT_ID env var
...
- argo-agent Copilot OAuth: `argo_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
