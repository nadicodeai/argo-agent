...
There are two ways to add a platform to the Argo gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.argo/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Argo code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `argo gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `argo_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Argo core.
...
And add it to the `argo-gateway` composite:

```python
"argo-gateway": {
    "includes": [..., "argo-your-platform"]
}
```
...
## 12. Status Display (`argo_cli/status.py`)
...
## 13. Gateway Setup Wizard (`argo_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ argo_cli/ toolsets.py \
  --include="*.py" -l | sort -u
