"""
Email platform adapter for the Argo gateway.

Allows users to interact with Argo by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "argo-agent" "version" "{_argo_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Argo Agent")
        ...
        msg_id = f"<argo-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
