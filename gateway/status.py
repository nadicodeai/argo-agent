...
The PID file lives at ``{ARGO_HOME}/gateway.pid``.  ARGO_HOME defaults to
``~/.argo`` but can be overridden via the environment variable.  This means
separate ARGO_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting ARGO_HOME."""
    home = get_argo_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("ARGO_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "argo" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Argo gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different ARGO_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``argo.service`` + ``argo-
# gateway.service``).
#
