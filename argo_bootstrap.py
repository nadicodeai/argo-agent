"""Windows UTF-8 bootstrap for Argo entry points.
...
should be imported at the very top of every Argo entry point
(``argo``, ``argo-agent``, ``argo-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import argo_bootstrap``
# (or ``from argo_bootstrap import apply_windows_utf8_bootstrap``) at
