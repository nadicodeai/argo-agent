...
Uses a file-based lock (~/.argo/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `argo update` reloads
# the module) fail with ModuleNotFoundError for argo_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from argo_constants import get_argo_home
from argo_cli._subprocess_compat import windows_hide_flags
from argo_cli.config import load_config, _expand_env_vars
from argo_time import now as _argo_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``argo tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from argo_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_argo_home: Path | None = None


def _get_argo_home() -> Path:
    """Resolve Argo home dynamically while preserving test monkeypatch hooks."""
    return _argo_home or get_argo_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    argo_home = _get_argo_home()
    lock_dir = argo_home / "cron"
    return lock_dir, lock_dir / ".tick.lock"


@contextmanager
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Argo profile.
    ...
    the scheduler's test/override hook and a context-local Argo home override
    both point at the resolved profile directory so _get_argo_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_argo_home() callers agree on the same home.
    ...
    global _argo_home
    prior_override = _argo_home
    env_snapshot = os.environ.copy()

    from argo_cli.profiles import normalize_profile_name, resolve_profile_env
    from argo_constants import reset_argo_home_override, set_argo_home_override
    ...
    override_token = None
    try:
        override_token = set_argo_home_override(profile_home)
        _argo_home = profile_home
        logger.info(
            "Job '%s': using Argo profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _argo_home = prior_override
        if override_token is not None:
            reset_argo_home_override(override_token)
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ARGO_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ARGO_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within ARGO_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against ARGO_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_argo_home() / "scripts"
    ...
    # escape — scripts MUST reside within ARGO_HOME/scripts/.
    try:
    ...
    run_env["ARGO_HOME"] = str(_get_argo_home())
    try:
        from argo_constants import get_subprocess_home
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _argo_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_argo_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_argo_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["ARGO_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed ARGO_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads ARGO_SESSION_PLATFORM
    #     and ARGO_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via ARGO_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     ARGO_SESSION_PLATFORM == "telegram".
    ...
    #     ARGO_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read ARGO_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the ARGO_CRON_AUTO_DELIVER_* vars set
    # below, so clearing ARGO_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "ARGO_CRON_AUTO_DELIVER_PLATFORM",
        "ARGO_CRON_AUTO_DELIVER_CHAT_ID",
        "ARGO_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_argo_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_argo_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["ARGO_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ARGO_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["ARGO_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("ARGO_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_argo_home() / "config.yaml")
        ...
        prefill_file = os.getenv("ARGO_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_argo_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject ARGO_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # ARGO_HOME. When a workdir is configured, also inject project
        ...
        # override via ARGO_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("ARGO_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ARGO_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_argo_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _argo_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _argo_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set ARGO_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ARGO_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ARGO_CRON_MAX_PARALLEL value; defaulting to unbounded")
        ...
        # Argo home override, scheduler _argo_home hook, and temporary
