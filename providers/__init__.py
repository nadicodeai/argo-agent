...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with argo-agent)
2. User plugins: ``$ARGO_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ARGO_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ARGO_HOME/plugins/model-providers/`` if it exists."""
    try:
        from argo_constants import get_argo_home

        d = get_argo_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ARGO_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_argo_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ARGO_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with argo-agent.
    ...
    # 2. User plugins — under $ARGO_HOME/plugins/model-providers/<name>/.
