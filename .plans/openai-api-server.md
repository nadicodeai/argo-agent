# OpenAI-Compatible API Server for Argo Agent
...
SSE streaming. By exposing this endpoint, argo-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  argo-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.argo/.env`
2. Run `argo gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with argo-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns argo-agent as a model) |
...
Authorization: Bearer argo-api-key-here
...
But argo-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "argo-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured argo-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of argo-agent's core prompt
...
Once implemented, argo-agent works as a drop-in backend for:
