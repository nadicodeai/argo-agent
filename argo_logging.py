"""Centralized logging setup for Argo Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.argo/logs/`` (profile-aware via ``get_argo_home()``).
...
from argo_constants import get_config_path, get_argo_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_argo_session_injector", False):
    ...
    _session_record_factory._argo_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``argo logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "argo_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Argo logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    argo_home
        Override for the Argo home directory.  Falls back to
        ``get_argo_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_argo_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._argo_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the argo group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from argo_cli.config import is_managed
