# Argo Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: argo-achievements plugin
...
### `GET /api/plugins/argo-achievements/achievements`
...
### `POST /api/plugins/argo-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/argo-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/argo-achievements/overview`
...
- Persist plugin data under:
  - `~/.argo/plugins/argo-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.argo/plugins/argo-achievements/scan_checkpoint.json`
...
- `~/.argo/plugins/argo-achievements/`
