# Argo Achievements Implementation Spec (Detailed)
...
- persisted runtime files:
  - `~/.argo/plugins/argo-achievements/state.json` (existing unlock state)
  - `~/.argo/plugins/argo-achievements/scan_snapshot.json` (new)
  - `~/.argo/plugins/argo-achievements/scan_checkpoint.json` (new)
