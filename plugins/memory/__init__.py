...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with argo-agent)
2. User-installed providers: ``$ARGO_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$ARGO_HOME/plugins/`` or None if unavailable."""
    try:
        from argo_constants import get_argo_home
        d = get_argo_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$ARGO_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"plugins.memory.{name}" if _is_bundled else f"_argo_user_memory.{name}"
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"plugins.memory.{active_provider}.cli" if _is_bundled else f"_argo_user_memory.{active_provider}.cli"
    try:
