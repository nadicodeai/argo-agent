...
Config file: `$ARGO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `argo` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `argo-{identity}` → `argo-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Argo profile:

```json
{
  "container_tag": "argo-{identity}"
}
```

For a profile named `coder`, this resolves to `argo-coder`. The default profile resolves to `argo-default`. Without `{identity}`, all profiles share the same container.
