...
def _load_supermemory_config(argo_home: str) -> dict:
    config = _default_config()
    config_path = Path(argo_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, argo_home: str) -> None:
    config_path = Path(argo_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `argo memory setup`.
        # All other options are documented for $ARGO_HOME/supermemory.json
    ...
    def save_config(self, values, argo_home):
        ...
        _save_supermemory_config(sanitized, argo_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from argo_constants import get_argo_home
        self._argo_home = kwargs.get("argo_home") or str(get_argo_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._argo_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "argo", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "argo_memory", "target": target, "type": "explicit_memory"},
