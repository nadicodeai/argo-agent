...
Resolution order for config file:
  1. $ARGO_HOME/honcho.json  (instance-local, enables isolated Argo instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Argo profile.

    Resolution order:
      1. ARGO_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``argo.<profile>``
      3. Fallback: ``"argo"`` (default profile)
    """
    explicit = os.environ.get("ARGO_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $ARGO_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.argo/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_argo_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_argo_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Argo system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.argo
    ...
    def from_global_config(
        ...
        Resolution: $ARGO_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Argo profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.argo block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Argo session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Argo session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Argo session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'argo honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run argo honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `argo honcho setup` to configure)."
        )
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            argo_cfg = load_config()
            honcho_cfg = argo_cfg.get("honcho", {})
