...
```bash
argo honcho setup    # full interactive wizard (cloud or local)
argo memory setup    # generic picker, also works
```

Or manually:
```bash
argo config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ARGO_HOME/honcho.json` | Profile-local (isolated Argo instances) |
| 2 | `~/.argo/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Argo profile: `argo` (default) or `argo.<profile>`.
...
| 4 | `per-session` strategy | Argo session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`argo-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`argo`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-argo-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening argo in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Argo session ID (timestamp + hex). Every `argo` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Argo profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`argo`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Argo profile: `argo` (default) or `argo.<profile>` (e.g. `argo -p coder` → host key `argo.coder`).
...
| `ARGO_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `argo honcho setup` | Full interactive setup wizard |
| `argo honcho status` | Show resolved config for active profile |
| `argo honcho enable` / `disable` | Toggle Honcho for active profile |
| `argo honcho mode <mode>` | Change recall or observation mode |
| `argo honcho peer --user <name>` | Update user peer name |
| `argo honcho peer --ai <name>` | Update AI peer name |
| `argo honcho tokens --context <N>` | Set context token budget |
| `argo honcho tokens --dialectic <N>` | Set dialectic max chars |
| `argo honcho map <name>` | Map current directory to a session name |
| `argo honcho sync` | Create host blocks for all Argo profiles |
