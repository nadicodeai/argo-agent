...
``{title, url, description, position}`` rows every other Argo web
provider produces.
...
prefers Argo-managed xAI Grok OAuth (via ``argo auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.argo/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``argo tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `argo auth` to sign in with "
