...
Auto-tracks and cleans up ephemeral files created during Argo Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ARGO_HOME` and `/tmp/argo-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ARGO_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ARGO_HOME` or `/tmp/argo-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ARGO_HOME/disk-cleanup/` is itself excluded
- `$ARGO_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
