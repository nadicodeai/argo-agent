"""disk_cleanup — ephemeral file cleanup for Argo Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ARGO_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ARGO_HOME and /tmp/argo-*
Never touches: ~/.argo/logs/ or any system directory.
"""
...
try:
    ...
    def get_argo_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ARGO_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".argo").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ARGO_HOME/logs/``."""
    return get_argo_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ARGO_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ARGO_HOME or ``/tmp/argo-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    argo_home = get_argo_home()
    try:
        path.resolve().relative_to(argo_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/argo-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("argo-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ARGO_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ARGO_HOME (but leave ARGO_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "argo-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(argo_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == argo_home:
                continue
            try:
                rel_parts = dirpath.relative_to(argo_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    argo_home = get_argo_home()
    try:
        rel = path.resolve().relative_to(argo_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "argo-agent",
        }:
            return None
        ...
        # Path isn't under ARGO_HOME (e.g. /tmp/argo-*) — fall through.
        pass
