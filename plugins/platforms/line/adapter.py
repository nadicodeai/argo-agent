"""
LINE Messaging API platform adapter for Argo Agent.
...
to Argo Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``ARGO_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            argo_home = Path.home().joinpath(".argo").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``argo status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``argo status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``argo setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.argo/.env`` via ``argo_cli.config``.
    """
    print()
    ...
    try:
        from argo_cli.config import get_env_var, set_env_var
    except ImportError:
        print("argo_cli.config not available; set LINE_* vars manually in ~/.argo/.env")
        return
