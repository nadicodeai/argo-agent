"""SimpleX Chat platform adapter (Argo plugin).
...
This adapter ships as a Argo platform plugin under
``plugins/platforms/simplex/``. The Argo plugin loader scans the
...
discoverable and `argo setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Argo — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"argo-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``argo setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.argo/.env`` via ``argo_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("argo_cli.config not available; set SIMPLEX_* vars manually in ~/.argo/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `argo gateway status` without
