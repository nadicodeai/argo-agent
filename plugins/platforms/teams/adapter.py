"""
Microsoft Teams platform adapter for Argo Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Argo"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        argo_action = data.get("argo_action", "")
        session_key = data.get("session_key", "")

        if not argo_action or not session_key:
        ...
        choice = choice_map.get(argo_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="argo_approve",
                    data={**btn_data_base, "argo_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="argo_approve",
                    data={**btn_data_base, "argo_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="argo_approve",
                    data={**btn_data_base, "argo_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="argo_approve",
                    data={**btn_data_base, "argo_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Argo\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.argo/.env")
