...
# ``model_tools`` import time, so ``argo status``, ``argo chat``, etc.
...
# ``argo_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Argo is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a argo MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ARGO_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _argo_home = _get_argo_home()
        except (ModuleNotFoundError, ImportError):
            _argo_home = _Path.home() / ".argo"
        self._thread_count_store = _ThreadCountStore(
            _argo_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Argo is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ARGO_HOME", str(_Path.home() / ".argo"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ARGO_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a argo MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Argo is thinking…' marker message.
        ...
        "Argo is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Argo is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Argo is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Argo is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Argo is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``argo setup``.

    The setup wizard at ``argo_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.argo/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. argo-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.argo/.env")
    print_info("Restart the gateway: argo gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``argo cron`` running as a
    separate process from ``argo gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Argo plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'argo-agent[google_chat]'",
        ...
        # the "Argo is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Argo is thinking…' marker message "
