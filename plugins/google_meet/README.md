...
Let the argo agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where argo runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `argo meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `argo meet` CLI |
...
| `cli.py` | `argo meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ARGO_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `argo meet node {run,list,approve,remove,status,ping}` |
...
argo meet install                                      # pip + Chromium
argo meet setup                                        # preflight
argo meet auth                                         # optional
argo meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
argo meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.argo/.env
argo meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
argo meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
argo meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.argo/.env
argo meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, argo will **not** switch your system audio input automatically — the
...
argo plugins enable google_meet
argo meet node run --display-name my-mac --host 0.0.0.0 --port 18789
