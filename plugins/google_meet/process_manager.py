...
session-scoped state file under ``$ARGO_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $ARGO_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_argo_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this argo install, leave it first —
    ...
    env = os.environ.copy()
    env["ARGO_MEET_URL"] = url
    env["ARGO_MEET_OUT_DIR"] = str(out)
    env["ARGO_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["ARGO_MEET_HEADED"] = "1"
    if auth_state:
        env["ARGO_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["ARGO_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if ARGO_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["ARGO_MEET_MODE"] = mode
    if realtime_model:
        env["ARGO_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["ARGO_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["ARGO_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["ARGO_MEET_REALTIME_KEY"] = realtime_api_key
