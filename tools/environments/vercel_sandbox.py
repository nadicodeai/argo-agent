...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Argo'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``ARGO_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_argo_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.argo"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_argo = (
            "/.argo"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.argo"
        )
        archive_member = remote_argo.lstrip("/")
        remote_tar = f"/tmp/.argo_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
