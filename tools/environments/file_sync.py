...
_FORCE_SYNC_ENV = "ARGO_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.argo") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.argo to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``ARGO_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, argo_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.argo/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="argo-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.argo/skills/a.md`` →
        ``~/.argo/skills/a.md``, a new remote file at
        ``/root/.argo/skills/b.md`` maps to ``~/.argo/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
