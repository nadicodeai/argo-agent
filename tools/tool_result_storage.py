...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/argo-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/argo-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/argo-results"
HEREDOC_MARKER = "ARGO_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/argo-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"ARGO_PERSIST_{uuid.uuid4().hex[:8]}"
