...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``argo tools`` repaint, tool-registration scans,
    ...
    2. ``~/.argo/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_argo_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.argo/.env`` first, then ``os.environ``.

    Wraps :func:`argo_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from argo_cli.config import get_env_value as _argo_get_env_value

        value = _argo_get_env_value(name)
...
def argo_xai_user_agent() -> str:
    """Return a stable Argo-specific User-Agent for xAI HTTP calls."""
    try:
        from argo_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Argo-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Argo-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``argo_cli.config.get_env_value`` so keys
    stored in ``~/.argo/.env`` (the standard Argo location) are honored —
