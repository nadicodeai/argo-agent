...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ARGO_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("ARGO_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set ARGO_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind ARGO_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds ARGO_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("ARGO_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $ARGO_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_ARGO_ENV_PATH = (
    r'(?:~\/\.argo/|'
    r'(?:\$home|\$\{home\})/\.argo/|'
    r'(?:\$argo_home|\$\{argo_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bargo\s+gateway\s+(stop|restart)\b', "stop/restart argo gateway (kills running agents)"),
    (r'\bargo\s+update\b', "argo update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart argo-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart argo-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(argo|gateway|cli\.py)\b', "kill argo/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill argo` but not
    # `kill -9 $(pgrep -f argo)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("ARGO_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("ARGO_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("ARGO_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("ARGO_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("ARGO_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("ARGO_EXEC_ASK")
