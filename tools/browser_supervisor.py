...
One ``CDPSupervisor`` runs per Argo ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "argo-dialog-bridge.invalid"
...
(() => {
  if (window.__argoDialogBridgeInstalled) return;
  window.__argoDialogBridgeInstalled = true;
