...
provider** — turn on a backend by enabling a plugin (``argo plugins
enable video_gen/<name>``) and selecting it in ``argo tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/argo-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `argo tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `argo plugins list` to see "
            f"installed video gen backends, or `argo tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `argo tools` → "
...
# `argo tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`argo tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `argo tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
