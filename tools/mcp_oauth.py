...
This module provides the glue:
    - ``ArgoTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Argo Agent"
"""
...
from argo_constants import secure_parent_dir
...
def _get_token_dir() -> Path:
    ...
    Uses ARGO_HOME so each profile gets its own OAuth tokens.
    Layout: ``ARGO_HOME/mcp-tokens/``
    """
    try:
        from argo_constants import get_argo_home
        base = Path(get_argo_home())
    except ImportError:
        base = Path(os.environ.get("ARGO_HOME", str(Path.home() / ".argo")))
...
# ---------------------------------------------------------------------------
# ArgoTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ArgoTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ARGO_HOME/mcp-tokens/<server_name>.json         -- tokens
        ARGO_HOME/mcp-tokens/<server_name>.client.json   -- client info
        ARGO_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Argo records an absolute wall-clock ``expires_at`` alongside the
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://argo-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
