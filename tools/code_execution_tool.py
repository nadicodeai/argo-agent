...
Lets the LLM write a Python script that calls Argo tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `argo_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `argo_tools.py` with file-based RPC stubs
...
# available on every platform Argo itself runs on.
...
def generate_argo_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the argo_tools.py stub module.
...
"""Auto-generated Argo tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["ARGO_RPC_SOCKET"]
...
"""Auto-generated Argo tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("ARGO_RPC_DIR") or os.path.join(tempfile.gettempdir(), "argo_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated argo_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/argo_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_argo_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/argo_tools.py", tools_src)
        ...
        env_prefix = (
            f"ARGO_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("ARGO_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with argo_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="argo_sandbox_")
    ...
    # selector from ARGO_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"argo_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated argo_tools module.
        ...
        tools_src = generate_argo_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "argo_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  ARGO_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["ARGO_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the argo-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from argo_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _argo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _argo_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # ARGO_TIMEZONE is an internal Argo setting and must not leak
        # into child processes.
        _tz_name = os.getenv("ARGO_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("ARGO_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {ARGO_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.argo/.env')).
...
def _load_config() -> dict:
    ...
    ``argo --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (argo-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``argo tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and argo-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.argo/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Argo tools programmatically. "
        ...
        f"Available via `from argo_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into argo_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from argo_tools import {import_str}` "
