...
it is automatically downloaded from GitHub releases to $ARGO_HOME/bin/tirith.
...
def _get_argo_home() -> str:
    """Return the Argo home directory, respecting ARGO_HOME env var."""
    return str(get_argo_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_argo_home(), ".tirith-install-failed")
...
def _argo_bin_dir() -> str:
    """Return $ARGO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_argo_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ARGO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_argo_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ARGO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ARGO_HOME/bin/tirith
    ...
    argo_bin = os.path.join(_argo_bin_dir(), "tirith")
    if os.path.isfile(argo_bin) and os.access(argo_bin, os.X_OK):
        _resolved_path = argo_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return argo_bin
