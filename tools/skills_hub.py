#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Argo Skills Hub.
...
Used by argo_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            argo_meta = metadata.get("argo", {})
            if isinstance(argo_meta, dict):
                tags = argo_meta.get("tags", [])
        if not tags:
...
def _load_argo_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ARGO_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ARGO_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ARGO_INDEX_CACHE_FILE.stat().st_mtime
            if age < ARGO_INDEX_TTL:
                return json.loads(ARGO_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ARGO_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Argo index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Argo index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ARGO_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ARGO_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ArgoIndexSource(SkillSource):
    """Skill source backed by the centralized Argo Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "argo-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "argo-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ArgoIndexSource(auth=auth), # Centralized index (search + resolved install paths)
