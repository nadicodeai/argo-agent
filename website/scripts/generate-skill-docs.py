...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/argo-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    argo_meta = (fm.get("metadata") or {}).get("argo") or {}
    tags = argo_meta.get("tags") or []
    related = argo_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `argo skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Argo Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Argo ships with a large built-in skill library copied into `~/.argo/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Argo also syncs bundled skills on `argo update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.argo/skills/` tree, it is still shipped with Argo; restore it with `argo skills reset <name> --restore`.",
        "",
