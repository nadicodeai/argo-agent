...
The ACP adapter wraps Argo' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
argo acp / argo-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.argo/.env
  -> configure stderr logging
  -> construct ArgoACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'argo-agent[acp]==<version>' argo-acp`, pointed at the `argo-agent` PyPI release.
...
- `allow_once` -> Argo `once`
- `allow_always` -> Argo `always`
- reject options -> Argo `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Argo tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["argo-acp"])
...
Instead it reuses Argo' runtime resolver:

- `acp_adapter/auth.py`
- `argo_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Argo provider/credentials. It also always advertises a terminal setup auth method (`argo-setup`, args `--setup`) so first-run registry clients can open Argo' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.argo/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `argo-acp` toolset definition
- `argo_cli/main.py` — `argo acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `argo-acp` script
