...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Argo already has applies. The plugin never sees a
...
[`argo-example-plugins`](https://github.com/NousResearch/argo-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
argo-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/NousResearch/argo-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Argo provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Argo provider
        # (must be one Argo already knows about — same names as
        # `argo model` and config.yaml model.provider).
        ...
        # plugin sends — Argo does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.argo/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Argo' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Argo subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/argo-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/argo-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/argo-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/argo-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
