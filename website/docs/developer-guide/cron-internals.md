...
description: "How Argo stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `argo_cli/cron.py` | CLI `argo cron` subcommands |
...
Jobs are stored in `~/.argo/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `argo cron` commands are run or during active CLI sessions.
...
```python
# ~/.argo/scripts/check_competitors.py
...
2. **Environment variable** — `ARGO_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.argo/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `argo cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `argo cron` CLI provides direct job management:

```bash
argo cron list                    # Show all jobs
argo cron create                  # Interactive job creation (alias: add)
argo cron edit <job_id>           # Edit job configuration
argo cron pause <job_id>          # Pause a running job
argo cron resume <job_id>         # Resume a paused job
argo cron run <job_id>            # Trigger immediate execution
argo cron remove <job_id>         # Delete a job
```
