...
This page covers all commands related to [Argo profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `argo profile` without a subcommand shows help.
...
```bash
argo profile use <name>
```

Sets `<name>` as the active profile. All subsequent `argo` commands (without `-p`) will use this profile.
...
argo profile use default
```

## `argo profile create`

```bash
argo profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `argo profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `argo update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
argo profile create mybot

# Clone config only from current profile
argo profile create work --clone

# Clone everything from current profile
argo profile create backup --clone-all

# Clone config from a specific profile
argo profile create work2 --clone --clone-from work
```

## `argo profile describe`

```bash
argo profile describe [<name>] [options]
```
...
argo profile describe researcher

# Set it explicitly
argo profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
argo profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
argo profile describe --all --auto
```

## `argo profile delete`

```bash
argo profile delete <name> [options]
```
...
```bash
argo profile delete mybot
argo profile delete mybot --yes
```
...
```bash
argo profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Argo home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ argo profile show work
Profile: work
Path:    ~/.argo/profiles/work
...
```bash
argo profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Argo installation.
...
```bash
argo profile alias work
# Creates/updates ~/.local/bin/work

argo profile alias work --name mywork
# Creates ~/.local/bin/mywork

argo profile alias work --remove
...
```bash
argo profile rename <old-name> <new-name>
```
...
```bash
argo profile rename mybot assistant
# ~/.argo/profiles/mybot → ~/.argo/profiles/assistant
...
```bash
argo profile export <name> [options]
```
...
```bash
argo profile export work
# Creates work.tar.gz in the current directory

argo profile export work -o ./work-2026-03-29.tar.gz
```

## `argo profile import`

```bash
argo profile import <archive> [options]
```
...
```bash
argo profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

argo profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`argo profile export` / `import` are still the right commands for
...
### `argo profile install`

```bash
argo profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `argo -p telemetry`). |
...
argo profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
argo profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
argo profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
argo profile install ./telemetry/
```

### `argo profile update`

```bash
argo profile update <name> [--force-config] [--yes]
```
...
```bash
argo profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Argo version, author, env var requirements, the source URL/path, and
...
`argo profile list` also shows the distribution name and version in a
`Distribution` column, and `argo profile show <name>` / `delete <name>`
...
argo profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
argo profile install https://github.com/your-org/internal-assistant.git
```
...
`argo_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Argo version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Argo can clone from.
3. Tell recipients to run `argo profile install <your-repo-url>`.
...
```bash
argo -p <name> <command> [options]
argo --profile <name> <command> [options]
```

Global flag to run any Argo command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
argo -p work chat -q "Check the server status"
argo --profile dev gateway start
argo -p personal skills list
argo -p work config edit
```

## `argo completion`

```bash
argo completion <shell>
```
...
- `argo profile <TAB>` — subcommands (list, use, create, etc.)
- `argo profile use <TAB>` — profile names
- `argo -p <TAB>` — profile names
