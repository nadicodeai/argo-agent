...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Argo Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Argo setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Argo Agent Tutorials & Use Cases](https://www.youtube.com/channel/UCqB1bhMwGsW-yefBxYwFCCg) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Argo Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Argo for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Argo working on my machine | `argo setup` | Run a real chat and verify it responds |
| I already know my provider | `argo model` | Save the config, then start chatting |
| I want a bot or always-on setup | `argo gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `argo model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `argo model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Argo cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Argo Agent

**Option A — pip (simplest):**

```bash
pip install argo-agent
argo postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
The single most important setup step. Use `argo model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `argo model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `argo model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `argo model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `argo model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `argo model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `argo model` (requires `copilot` CLI + `copilot login`) |
...
Argo Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `argo model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Argo separates secrets from normal config:

- **Secrets and tokens** → `~/.argo/.env`
- **Non-secret settings** → `~/.argo/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
argo config set model anthropic/claude-opus-4.6
argo config set terminal.backend docker
argo config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
argo            # classic CLI
argo --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Argo ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `argo` vs `argo --tui`.
:::
...
```bash
argo --continue    # Resume the most recent session
argo -c            # Short form
```
...
```bash
argo gateway setup    # Interactive platform configuration
```
...
- `argo tools` — tune tool access per platform
- `argo skills` — browse and install reusable workflows
...
```bash
argo config set terminal.backend docker    # Docker isolation
argo config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Argo install directory (the curl installer placed it at
# ~/.argo/argo-agent on Linux/macOS or %LOCALAPPDATA%\argo\argo-agent on Windows):
cd ~/.argo/argo-agent
...
(If you installed without `[all]`, run `cd ~/.argo/argo-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Argo opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `argo model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `argo gateway setup` and check `argo gateway status` |
| `argo --continue` can't find old session | Switched profiles or session never saved | Check `argo sessions list` and confirm you're in the right profile |
...
| `argo doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `argo model`
3. `argo setup`
4. `argo sessions list`
5. `argo --continue`
6. `argo gateway status`
...
| `argo tools` | Configure which tools are enabled per platform |
| `argo setup` | Full setup wizard (configures everything at once) |
| `argo doctor` | Diagnose issues |
| `argo update` | Update to latest version |
| `argo gateway` | Start the messaging gateway |
| `argo --continue` | Resume last session |
