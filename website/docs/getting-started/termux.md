...
description: "Run Argo Agent directly on an Android phone with Termux"
---

# Argo on Android with Termux

This is the tested path for running Argo Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Argo from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `argo` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `argo` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/argo" "$PREFIX/bin/argo"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `argo` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.argo/.env`.
...
### `argo doctor` says ripgrep or Node is missing
