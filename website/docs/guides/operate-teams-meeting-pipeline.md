...
```bash
argo teams-pipeline token-health
argo teams-pipeline token-health --force-refresh
```
...
```bash
argo teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
argo teams-pipeline maintain-subscriptions
argo teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Argo cron (recommended if you already run the Argo gateway)

Argo ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.argo/scripts/`. First create the script:

```bash
mkdir -p ~/.argo/scripts
cat > ~/.argo/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec argo teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.argo/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
argo cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/argo-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Argo Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=argo
EnvironmentFile=/etc/argo/env
ExecStart=/usr/local/bin/argo teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/argo-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Argo Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now argo-teams-pipeline-maintain.timer
systemctl list-timers argo-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/argo teams-pipeline maintain-subscriptions >> /var/log/argo/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.argo/.env` at the top of a wrapper script that crontab calls.
...
```bash
argo teams-pipeline subscriptions   # should show expirationDateTime advanced
argo teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
argo teams-pipeline list
argo teams-pipeline list --status failed
argo teams-pipeline show <job-id>
```

### Replay a stored job

```bash
argo teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
argo teams-pipeline fetch --meeting-id <meeting-id>
argo teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `argo teams-pipeline maintain-subscriptions --dry-run`
- inspect `argo teams-pipeline list --status failed`
...
- run `argo teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `argo teams-pipeline run`
...
- [ ] `argo teams-pipeline validate` returns an OK snapshot
- [ ] `argo teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Argo cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
