...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Argo Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

Argo Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and Argo automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Argo — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Argo opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.argo/auth.json` and refreshed automatically before they expire.
...
```bash
argo auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, Argo detects the remote environment and prints the authorization URL instead of opening a browser.
...
argo auth add xai-oauth --no-browser
...
If you don't have a regular SSH client (e.g. you're running Argo inside GCP Cloud Shell, GitHub Codespaces, AWS EC2 Instance Connect, Gitpod, or another browser-based console), the `ssh -L` recipe above isn't available. Use `--manual-paste` instead — Argo skips the loopback listener and lets you paste the failed callback URL straight from your browser:

```bash
argo auth add xai-oauth --manual-paste
# Or via the model picker:
argo model --manual-paste
```

See [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md#browser-only-remote-cloud-shell--codespaces--ec2-instance-connect) for the full walkthrough. Regression fix for [#26923](https://github.com/NousResearch/argo-agent/issues/26923).

## How the Login Works

1. Argo opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Argo and the tokens are saved to `~/.argo/auth.json`.
4. From then on, Argo refreshes the access token in the background — you stay signed in until you `argo auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
argo config set model.default grok-4.3
argo config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.argo/config.yaml` will contain:
...
```bash
argo --provider xai-oauth        # canonical
argo --provider grok-oauth       # alias
argo --provider x-ai-oauth       # alias
argo --provider xai-grok-oauth   # alias
```
...
| `ARGO_INFERENCE_PROVIDER` | Force the active provider at runtime, e.g. `ARGO_INFERENCE_PROVIDER=xai-oauth argo`. |

## Troubleshooting

### Token expired — not re-logging in automatically

Argo refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Argo surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Argo marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `argo auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Argo raises a timeout error.

**Fix:** re-run `argo auth add xai-oauth` (or `argo model`). The flow starts fresh.

### State mismatch (possible CSRF)

Argo detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Argo prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
This is **not** a stale-token problem — re-running `argo model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/NousResearch/argo-agent/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
argo config set model.provider xai
```
...
**Fix:** run `argo model` and pick the xAI Grok OAuth provider, or run `argo auth add xai-oauth`.
...
```bash
argo auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `argo auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `argo auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Argo is on a different machine than your browser
