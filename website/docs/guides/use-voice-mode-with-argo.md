---
sidebar_position: 8
title: "Use Voice Mode with Argo"
description: "A practical guide to setting up and using Argo voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Argo
...
- you want Argo sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Argo.
...
## Step 1: make sure normal Argo works first
...
Argo supports both local and cloud speech stacks.
...
### If you use `argo setup`

If you choose NeuTTS in the setup wizard, Argo checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Argo transcribes and responds
...
- asking Argo to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Argo loop.

## Tuning CLI behavior

### Silence threshold

If Argo starts/stops too aggressively, tune:
...
Argo stays a normal chat bot, but can speak replies.
...
- you want Argo to function like a portable research or ops assistant
...
Argo joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Argo detects speech boundaries
- transcripts are posted in the associated text channel
- Argo responds in text and audio
...
1. get text Argo working
2. install `argo-agent[voice]`
