---
sidebar_position: 6
title: "Use MCP with Argo"
description: "A practical guide to connecting MCP servers to Argo Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Argo

This guide shows how to actually use MCP with Argo Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Argo tool
- you want Argo to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Argo to internal APIs, databases, or company systems without modifying Argo core

Do not use MCP when:
- a built-in Argo tool already solves the job well
...
- Argo remains the agent
- MCP servers contribute tools
- Argo discovers those tools at startup or reload time
...
If you installed Argo with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Argo banner/status should show MCP integration when configured
- ask Argo what tools it has available
...
## WSL2: bridge Argo in WSL to Windows Chrome

This is the practical setup when:

- Argo runs inside WSL2
...
In this setup, Argo does **not** connect to Chrome directly. Instead:

- Argo runs in WSL
- Argo starts a local stdio MCP server
...
```text
Argo (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Argo stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Argo core browser transport
...
```bash
argo mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
argo mcp test chrome-devtools-win
```

Then start a fresh Argo session or run:
...
Once loaded, Argo can use the MCP-prefixed browser tools directly. For example:
...
If Argo runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Argo from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Argo from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Argo:
...
So Argo will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Argo to reason over a bounded workspace.
...
Now Argo can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Argo core.
...
Because Argo now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
