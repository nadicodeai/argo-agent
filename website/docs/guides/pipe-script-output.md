...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `argo send`."
---

# Pipe Script Output to Messaging Platforms

`argo send` is a small, scriptable CLI that pushes a message to any
messaging platform Argo is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | argo send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `argo
...
argo send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | argo send --to telegram:-1001234567890

# Send a file
argo send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
argo send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
argo send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
argo send --list

# Filter by platform
argo send --list telegram
```
...
Any platform Argo ships adapters for works as a target:
...
`argo send` resolves the message body in this order:

1. **Positional argument** — `argo send --to telegram "hi"`
2. **`--file PATH`** — `argo send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | argo send --to telegram`

When stdin is a TTY (no pipe), Argo does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  argo send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `argo send` reuses your Argo config, the same script works on
any host where Argo is installed — no need to export bot tokens into
...
`argo send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  argo send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | argo send \
...
```bash
./train.py --epochs 200 && \
  argo send --to telegram "training done" || \
  argo send --to telegram "training failed (exit $?)"
```
...
argo send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(argo send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `argo send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `argo send` calls
the platform's REST endpoint directly using credentials from
`~/.argo/.env` and `~/.argo/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `argo gateway start` and retry.
...
The listing is built from `~/.argo/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`argo gateway
...
| Approach | Multi-platform | Reuses Argo creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `argo send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`argo send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `argo send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `argo send` shares with cron delivery.
