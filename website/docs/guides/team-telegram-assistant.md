...
This tutorial walks you through setting up a Telegram bot powered by Argo Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Argo Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.argo/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Argo itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Argo Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_argo_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Argo Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u argo-gateway -f
...
argo gateway start
argo gateway stop
tail -f ~/.argo/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `argo gateway install` to update the plist.
:::
...
```bash
argo gateway stop && argo gateway start
```
...
argo pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.argo/.env`:
...
Control how much detail the bot shows when using tools. In `~/.argo/config.yaml`:
...
Customize how the bot communicates by editing `~/.argo/SOUL.md`:

For a full guide, see [Use SOUL.md with Argo](/docs/guides/use-soul-with-argo).
...
```bash
# From the CLI
argo cron list          # View all scheduled jobs
argo cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u argo-gateway` (Linux) or `~/.argo/logs/gateway.log` (macOS) |
| Cron job output | `~/.argo/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.argo/cron/jobs.json` |
| Pairing data | `~/.argo/pairing/` |
| Session history | `~/.argo/sessions/` |
