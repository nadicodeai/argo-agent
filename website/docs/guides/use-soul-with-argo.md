---
sidebar_position: 7
title: "Use SOUL.md with Argo"
description: "How to use SOUL.md to shape Argo Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Argo

`SOUL.md` is the **primary identity** for your Argo instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Argo to feel like the same assistant every time you talk to it — or if you want to replace the Argo persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Argo should be
- what Argo should avoid stylistically
- how Argo should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Argo is and how Argo speaks
...
Argo now uses only the global SOUL file for the current instance:

```text
~/.argo/SOUL.md
```

If you run Argo with a custom home directory, it becomes:
...
Argo automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Argo does not overwrite it
- if the file exists but is empty, Argo adds nothing from it to the prompt

## How Argo uses it

When Argo starts a session, it reads `SOUL.md` from `ARGO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Argo feels.
...
Argo already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Argo should sound.

# Avoid
What Argo should not do.

# Defaults
How Argo should behave when ambiguity appears.
```
...
Then restart Argo or start a new session.
...
### I edited SOUL.md but Argo still sounds the same

Check:
- you edited `~/.argo/SOUL.md` or `$ARGO_HOME/SOUL.md`
...
### Argo is ignoring parts of my SOUL.md
