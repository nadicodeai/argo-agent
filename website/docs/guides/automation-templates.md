...
argo cron create "0 2 * * *" \
  "You are a project manager triaging the nadicodeai/argo-agent GitHub repo.

1. Run: gh issue list --repo nadicodeai/argo-agent --state open --json number,title,labels,author,createdAt --limit 30
...
argo cron create "0 9 * * 1" \
  "Scan the nadicodeai/argo-agent repo for documentation drift.

1. Run: gh pr list --repo nadicodeai/argo-agent --state merged --json number,title,files,mergedAt --limit 30
