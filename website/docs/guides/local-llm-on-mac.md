...
Both expose an OpenAI-compatible `/v1/chat/completions` endpoint. Argo works with either one — just point it at `http://localhost:8080` or `http://localhost:8000`.
...
Argo automatically detects local endpoints (localhost, LAN IPs) and relaxes its streaming timeouts. No configuration needed for most setups.
...
ARGO_STREAM_READ_TIMEOUT=1800
```

| Timeout | Default | Local auto-adjustment | Env var override |
|---------|---------|----------------------|------------------|
| Stream read (socket-level) | 120s | Raised to 1800s | `ARGO_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Disabled entirely | `ARGO_STREAM_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | No change needed | `ARGO_API_TIMEOUT` |
