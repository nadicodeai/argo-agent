...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.argo/.env` at the end.
...
4. Fill in:
   - **Name:** `Argo Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `argo-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the Argo app
New-CsApplicationAccessPolicy `
  -Identity "Argo-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Argo meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Argo-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.argo/.env`:
...
Argo ships a Graph auth smoke-test. From your Argo install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.argo/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `argo gateway restart`.
