...
title: "Git Worktrees"
description: "Run multiple Argo agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Argo Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Argo so each session has a clean, isolated working directory.

## Why Use Worktrees with Argo?

Argo treats the **current working directory** as the project root:

- CLI: the directory where you run `argo` or `argo chat`
...
git worktree add ../repo-feature feature/argo-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/argo-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Argo there:

```bash
cd ../repo-feature

# Start Argo in the worktree
argo
```
...
git worktree add ../repo-experiment-a feature/argo-a
git worktree add ../repo-experiment-b feature/argo-b
```
...
- Works on its own branch (`feature/argo-a` vs `feature/argo-b`).
...
- Argo checkpoint data under `~/.argo/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Argo experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/argo-checkpoints-docs`, `feature/argo-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Argo from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `argo -w` (Automatic Worktree Mode)

Argo has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `argo/argo-<hash>`).
...
```bash
argo -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `argo -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Argo session its own clean checkout.
