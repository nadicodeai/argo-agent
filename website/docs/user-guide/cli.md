...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Argo Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Argo Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
Argo also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `argo --tui` — see the [TUI](tui.md) guide.
:::
...
argo chat --model "anthropic/claude-sonnet-4"

# With a specific provider
argo chat --provider nous        # Use Nous Portal
argo chat --provider openrouter  # Force OpenRouter

# With specific toolsets
argo chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
argo -s argo-agent-dev,github-auth
argo chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
argo --continue             # Resume the most recent CLI session (-c)
argo --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
argo chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
argo -w                         # Interactive mode in worktree
argo -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Argo CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Argo CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ARGO_YOLO_MODE` is on (either `argo --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`argo -c` or `argo --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Argo to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Argo supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Argo replies |
...
```bash
argo -s argo-agent-dev,github-auth
argo chat -s github-pr-workflow -s github-auth
```

Argo loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.argo/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.argo/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Argo..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Argo — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Argo is working, Argo prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Argo to the background — just like any terminal process. The shell prints a confirmation:

```
Argo Agent has been suspended. Run `fg` to bring Argo Agent back.
```
...
Resume this session with:
  argo --resume 20260225_143052_a1b2c3
...
```bash
argo --continue                          # Resume the most recent CLI session
argo -c                                  # Short form
argo -c "my project"                     # Resume a named session (latest in lineage)
argo --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
argo --resume "refactoring auth"         # Resume by title
argo -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `argo sessions rename <id> <title>` from the command line. Use `argo sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Argo's SQLite state database under `~/.argo/state.db`. The database keeps:
...
Argo immediately confirms the task and gives you back the prompt:
