...
Connect Argo to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Argo and the BlueBubbles server
...
Or set environment variables directly in `~/.argo/.env`:
...
When someone messages your iMessage, Argo automatically sends them a pairing code. Approve it with:
```bash
argo pairing approve bluebubbles <CODE>
```
Use `argo pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.argo/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.argo/.env`):
```bash
...
Argo will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Argo
Argo → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Argo sends messages via the BlueBubbles REST API.
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.argo/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Argo resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `argo logs gateway` for webhook errors (or `argo logs -f` to follow in real-time)
