...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Argo Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Argo to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.argo/.env`:
...
All standard Argo commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.argo/logs/gateway.log`
...
4. Consider enabling verbose logging: `ARGO_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Argo automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ARGO_LOG_LEVEL=debug argo gateway
```
...
```bash
argo chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
