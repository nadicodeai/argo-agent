...
description: "Chat with Argo from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Argo from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Argo](/docs/guides/use-voice-mode-with-argo).
...
```bash
argo gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
argo gateway              # Run in foreground
argo gateway setup        # Configure messaging platforms interactively
argo gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo argo gateway install --system   # Linux only: install a boot-time system service
argo gateway start        # Start the default service
argo gateway stop         # Stop the default service
argo gateway status       # Check default service status
argo gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Argo Agent to the latest version |
...
Configure per-platform overrides in `~/.argo/gateway.json`:
...
argo pairing approve telegram XKGH5N7P

# Other pairing commands:
argo pairing list          # View pending + approved users
argo pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Argo appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.argo/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.argo/config.yaml`:
...
```bash
ARGO_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
argo gateway install               # Install as user service
argo gateway start                 # Start the service
argo gateway stop                  # Stop the service
argo gateway status                # Check status
journalctl --user -u argo-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo argo gateway install --system
sudo argo gateway start --system
sudo argo gateway status --system
journalctl -u argo-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Argo will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Argo installations on the same machine (with different `ARGO_HOME` directories), each gets its own systemd service name. The default `~/.argo` uses `argo-gateway`; other installations use `argo-gateway-<hash>`. The `argo gateway` commands automatically target the correct service for your current `ARGO_HOME`.
:::

### macOS (launchd)

```bash
argo gateway install               # Install as launchd agent
argo gateway start                 # Start the service
argo gateway stop                  # Stop the service
argo gateway status                # Check status
tail -f ~/.argo/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.argo.gateway.plist`. It includes three environment variables:
...
- **ARGO_HOME** — scopes the gateway to your Argo installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `argo gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ARGO_HOME` directory gets its own launchd label. The default `~/.argo` uses `ai.argo.gateway`; other installations use `ai.argo.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `argo-cli` | Full access |
| Telegram | `argo-telegram` | Full tools including terminal |
| Discord | `argo-discord` | Full tools including terminal |
| WhatsApp | `argo-whatsapp` | Full tools including terminal |
| Slack | `argo-slack` | Full tools including terminal |
| Google Chat | `argo-google_chat` | Full tools including terminal |
| Signal | `argo-signal` | Full tools including terminal |
| SMS | `argo-sms` | Full tools including terminal |
| Email | `argo-email` | Full tools including terminal |
| Home Assistant | `argo-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `argo-mattermost` | Full tools including terminal |
| Matrix | `argo-matrix` | Full tools including terminal |
| DingTalk | `argo-dingtalk` | Full tools including terminal |
| Feishu/Lark | `argo-feishu` | Full tools including terminal |
| WeCom | `argo-wecom` | Full tools including terminal |
| WeCom Callback | `argo-wecom-callback` | Full tools including terminal |
| Weixin | `argo-weixin` | Full tools including terminal |
| BlueBubbles | `argo-bluebubbles` | Full tools including terminal |
| QQBot | `argo-qqbot` | Full tools including terminal |
| Yuanbao | `argo-yuanbao` | Full tools including terminal |
| Microsoft Teams | `argo-teams` | Full tools including terminal |
| API Server | `argo-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `argo-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.argo/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
