...
sidebar_position: 4
title: "Slack"
description: "Set up Argo Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Argo Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Argo instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Argo uses the modern
...
The fastest path is to paste a manifest Argo generates for you. It
...
### Option A: From a Argo-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   argo slack manifest --write
   ```
   This writes `~/.argo/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Argo Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Argo can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `argo-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Argo configuration issue.
:::
...
Argo uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
argo gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
argo gateway              # Foreground
argo gateway install      # Install as a user service
sudo argo gateway install --system   # Linux only: boot-time system service
```
...
Every Argo command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Argo command with its description.

Under the hood: Argo ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/argo-agent/blob/main/argo_cli/commands.py)
...
When Argo adds new commands (e.g. after `argo update`), regenerate
...
3. Paste the new contents of `~/.argo/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/argo <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/argo btw run the tests` — Argo routes it the same way as `/btw
run the tests`. Free-form questions also work: `/argo what's the
...
Slack never delivers them to Argo.

As a workaround, Argo recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Argo treats it identically to the slash form and replies in the same
thread.
...
```bash
argo slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Argo behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Argo Agent what time is it?`). In channels, Argo replies in a thread attached to that message. |
| **Threads** | If you @mention Argo inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.argo/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Argo in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Argo will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Argo will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Argo Agent`).

---

## Multi-Workspace Support

Argo can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Argo also loads tokens from an **OAuth token file** at:

```
~/.argo/slack_tokens.json
```
...
- When a message arrives, Argo uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Argo Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Argo Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Argo now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Argo Agent`)
...
- Tokens should be stored in `~/.argo/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Argo config directory
