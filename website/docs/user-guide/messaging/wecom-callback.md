...
Connect Argo to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Argo supports two WeCom integration modes:
...
3. Argo decrypts the message, queues it for the agent
...
(Use `argo gateway start` only after `argo gateway install` has registered the systemd/launchd service.)
