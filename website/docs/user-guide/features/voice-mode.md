...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Argo Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Argo Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Argo](/docs/guides/use-voice-mode-with-argo).

## Prerequisites

Before using voice features, make sure you have:

1. **Argo Agent installed** — `pip install argo-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `argo model` or set your preferred provider credentials in `~/.argo/.env`
3. **A working base setup** — run `argo` to verify the agent responds to text before enabling voice

:::tip
The `~/.argo/` directory and default `config.yaml` are created automatically the first time you run `argo`. You only need to create `~/.argo/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/docs/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `argo setup --portal` wires both up at once.
:::
...
pip install "argo-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "argo-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "argo-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "argo-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`argo chat`) and the **TUI** (`argo --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.argo/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
argo                # Start the interactive CLI
```
...
1. Start the CLI with `argo` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.argo/config.yaml` (default: `ctrl+b`).
:::
...
```bash
argo gateway        # Start the gateway (connects to configured platforms)
argo gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@argobyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.argo/.env`:
```bash
...
```bash
argo gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.argo/.env`
...
- Review gateway logs: `tail -f ~/.argo/logs/gateway.log`
