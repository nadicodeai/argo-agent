...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Argo's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Argo instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
argo memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$ARGO_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `argo` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Argo syncs them back at session init.
...
The `argo honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `argo memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
argo honcho status          # Connection status, config, and key settings
argo honcho setup           # Redirects to `argo memory setup`
argo honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
argo honcho peer            # Show or update peer names + dialectic reasoning level
argo honcho mode            # Show or set recall mode (hybrid/context/tools)
argo honcho tokens          # Show or set token budget for context and dialectic
argo honcho identity        # Seed or show the AI peer's Honcho identity
argo honcho sync            # Sync Honcho config to all existing profiles
argo honcho peers           # Show peer identities across all profiles
argo honcho sessions        # List known Honcho session mappings
argo honcho map             # Map current directory to a Honcho session name
argo honcho enable          # Enable Honcho for the active profile
argo honcho disable         # Disable Honcho for the active profile
argo honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `argo honcho`

If you previously used the standalone `argo honcho setup`:
...
No re-login or re-setup needed. Run `argo memory setup` and select "honcho" — the wizard detects your existing config.
