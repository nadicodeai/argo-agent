...
chat completions — use your Argo-managed provider subscription as their
...
| Use case | "Use Argo as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Argo stores
the refresh token in `~/.argo/auth.json` — the same place all Argo
...
```
Starting Argo proxy for Nous Portal
...
Model:      Argo-4-70B    # or Argo-4.3-36B, Argo-4-405B
```
...
```
Argo proxy upstream adapters

  [nous    ] Nous Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `argo login nous`. If you see
...
INFERENCE_TEXT_MODEL=Argo-4-70B
```
...
```bash
argo proxy start --host 0.0.0.0 --port 8645
```
...
`argo_cli/proxy/adapters/<provider>.py` and registering it in
