...
description: "Connect Argo Agent to external tool servers via MCP — and control exactly which MCP tools Argo loads"
---

# MCP (Model Context Protocol)

MCP lets Argo Agent connect to external tool servers so the agent can use tools that live outside Argo itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Argo to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Argo tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Argo to see
...
2. Add an MCP server to `~/.argo/config.yaml`:
...
4. Ask Argo to use the MCP-backed capability.
...
Argo will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Argo connects to directly.
...
- you do not want Argo spawning a local subprocess for that integration

## Basic configuration reference

Argo reads MCP config from `~/.argo/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Argo skips the server entirely |
...
For well-known MCP servers, `argo mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
argo mcp add codex --preset codex
```
...
You can pick any local name (`argo mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Argo registers MCP tools

Argo prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Argo sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Argo also registers utility tools around MCP resources and prompts:
...
- Argo only registers resource utilities if the MCP session actually supports resource operations
- Argo only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Argo, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Argo skips the server completely and does not even attempt a connection.
...
You can also separately disable Argo-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Argo does not create an empty runtime MCP toolset for that server.
...
Argo discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Argo when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Argo receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Argo does not blindly pass your full shell environment.
...
Then verify your config and restart Argo.
...
Because Argo now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Argo may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Argo via the `sampling/createMessage` protocol. This allows an MCP server to ask Argo to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Argo as an MCP server

In addition to connecting **to** MCP servers, Argo can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Argo's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Argo
- You want a single MCP server that bridges to all of Argo's connected messaging platforms at once
- You already have a running Argo gateway with connected platforms
...
Add Argo to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Argo in a specific location:

```json
{
  "mcpServers": {
    "argo": {
      "command": "/home/user/.argo/argo-agent/venv/bin/argo",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Argo-specific channel browser:
...
The MCP server includes a live event bridge that polls Argo's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
argo mcp serve              # Normal mode
argo mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Argo's session store (`~/.argo/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Argo agent itself.
...
- The embedded `argo mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Argo, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
