...
description: "Customize Argo Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Argo Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ARGO_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Argo is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Argo now seeds a default `SOUL.md` automatically in:

```text
~/.argo/SOUL.md
```

More precisely, it uses the current instance's `ARGO_HOME`, so if you run Argo with a custom home directory, it will use:
...
- Argo creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Argo loads `SOUL.md` only from `ARGO_HOME`
- Argo does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Argo falls back to a built-in default identity
...
If Argo loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ARGO_HOME`, the personality belongs to the Argo instance itself.

That also makes it easier to teach users:
- "Edit `~/.argo/SOUL.md` to change Argo' default personality."
...
- how Argo should handle uncertainty, disagreement, or ambiguity
...
## What Argo injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Argo falls back to a built-in default identity ("You are Argo Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Argo ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Argo, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Argo its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.argo/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.argo/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-argo)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Argo speaks
- `display.skin` and `/skin` affect how Argo looks in the terminal
