...
- Give it a name like "argo-agent"
- Select scopes:
...
- Give it a title like "argo-agent-&lt;machine-name>"
...
elif [ -f ~/.argo/.env ] && grep -q "^GITHUB_TOKEN=" ~/.argo/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.argo/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
