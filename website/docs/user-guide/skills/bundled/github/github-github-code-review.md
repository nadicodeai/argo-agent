...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Argo Agent\",
    \"comments\": [
...
```bash
source "${ARGO_HOME:-$HOME/.argo}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Argo Agent. Code looks clean — good test coverage, no security concerns."
