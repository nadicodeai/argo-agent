---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Argo Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Argo Agent"
---
...
Migrate a user's OpenClaw customization footprint into Argo Agent. Imports Argo-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `argo skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Argo Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Argo`, `Memory`, `Persona`, `Import` |
| Related skills | [`argo-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-argo-agent) |
...
# OpenClaw -> Argo Migration

Use this skill when a user wants to move their OpenClaw setup into Argo Agent with minimal manual cleanup.
...
```bash
argo claw migrate              # Full interactive migration
...
argo claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `argo setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_argo.py` to:

- import `SOUL.md` into the Argo home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Argo memory entries
- merge OpenClaw command approval patterns into Argo `command_allowlist`
- migrate Argo-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.argo/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Argo workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.argo/tts/`
- archive non-secret docs that do not have a direct Argo destination
...
- `scripts/openclaw_to_argo.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.argo/skills/migration/openclaw-migration/scripts/openclaw_to_argo.py`

Do not guess a shorter path like `~/.argo/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.argo/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Argo-compatible secrets, currently:
...
Argo CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Argo. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Argo workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Argo skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.argo/skills/migration/openclaw-migration/scripts/openclaw_to_argo.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.argo/skills/migration/openclaw-migration/scripts/openclaw_to_argo.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.argo/skills/migration/openclaw-migration/scripts/openclaw_to_argo.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.argo/skills/migration/openclaw-migration/scripts/openclaw_to_argo.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.argo/skills/migration/openclaw-migration/scripts/openclaw_to_argo.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.argo/skills/migration/openclaw-migration/scripts/openclaw_to_argo.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Argo unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Argo targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Argo destination. Unsupported auth blobs must still be reported as skipped.
...
- Argo persona state imported
- Argo memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.argo/skills/openclaw-imports/`
