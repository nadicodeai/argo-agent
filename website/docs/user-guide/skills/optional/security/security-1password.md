...
| | |
|---|---|
| Source | Optional — install with `argo skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Argo Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Argo terminal calls (desktop app flow only)
...
### Service Account (recommended for Argo)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.argo/.env` (the skill will prompt for this on first load).
...
## Argo Execution Pattern (desktop app flow)

Argo terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/argo-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/argo-op.sock"
