...
| | |
|---|---|
| Source | Optional — install with `argo skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Argo Agent |
| License | MIT |
...
import upstream classes into Argo itself.
...
```bash
mkdir -p ~/.argo/cache/darwinian-evolver && cd ~/.argo/cache/darwinian-evolver
...
```bash
cd ~/.argo/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.argo/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.argo/skills/research/darwinian-evolver
DE_DIR=~/.argo/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Argo core.
   Custom driver scripts under `~/.argo/skills/...` are user-side and fine.
