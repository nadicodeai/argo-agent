...
description: "Configure and use Honcho memory with Argo -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Argo -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `argo skills install official/autonomous-ai-agents/honcho` |
...
```bash
argo honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Argo profile's AI peer
...
Honcho models conversations as interactions between **peers**. Argo creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Argo instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Argo run |
| `global` | Single session across all directories |

Manual override: `argo honcho map my-project-name`
...
Each Argo profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block argo.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `argo.coder` host block in `honcho.json`
...
```bash
argo honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Argo when Honcho memory is active.
...
Config file: `$ARGO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `argo honcho setup`. Ensure `memory.provider: honcho` is in `~/.argo/config.yaml`.

### Memory not persisting across sessions
Check `argo honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `argo profile create <name> --clone`. For existing profiles: `argo honcho sync`.
...
| Command | Description |
|---------|-------------|
| `argo honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `argo honcho status` | Show resolved config, connection test, peer info for active profile |
| `argo honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `argo honcho disable` | Disable Honcho for the active profile |
| `argo honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `argo honcho peers` | Show peer identities across all profiles |
| `argo honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `argo honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `argo honcho sessions` | List known directory-to-session-name mappings |
| `argo honcho map <name>` | Map current working directory to a Honcho session name |
| `argo honcho identity` | Seed AI peer identity or show both peer representations |
| `argo honcho sync` | Create host blocks for all Argo profiles that don't have one yet |
| `argo honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Argo + Honcho |
| `argo memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `argo memory status` | Show active memory provider and config |
| `argo memory off` | Disable external memory provider |
