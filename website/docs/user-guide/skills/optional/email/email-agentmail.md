...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. argo-agent@agentmail.to).

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `argo skills install official/email/agentmail` |
...
Add to `~/.argo/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `argo-agent`)
   - The agent gets address: `argo-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "argo-outreach")
...
```
argo --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
