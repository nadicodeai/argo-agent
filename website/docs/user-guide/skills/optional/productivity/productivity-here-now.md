...
| | |
|---|---|
| Source | Optional — install with `argo skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ARGO_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ARGO_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ARGO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client argo
```
...
```bash
PUBLISH="${ARGO_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client argo
```
...
```bash
DRIVE="${ARGO_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `argo`)    |
