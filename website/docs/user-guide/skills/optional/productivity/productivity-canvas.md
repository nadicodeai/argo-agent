...
| | |
|---|---|
| Source | Optional — install with `argo skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Argo Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.argo/.env`:
...
```bash
CANVAS="python $ARGO_HOME/skills/productivity/canvas/scripts/canvas_api.py"
