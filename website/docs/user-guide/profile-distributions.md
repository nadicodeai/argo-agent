...
A **profile distribution** packages a complete Argo agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Argo agent meant sending someone:
...
```bash
argo profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `argo profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Argo in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`argo profile export` / `import`](../reference/profile-commands.md#argo-profile-export) — that's what those are for.
...
```bash
argo profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.argo/profiles/research-bot/SOUL.md
...
Create `~/.argo/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
argo_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Recipients who run `argo profile update research-bot` will pull the latest.
...
```bash
argo profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.argo/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
```bash
# GitHub shorthand
argo profile install github.com/you/research-bot

# Full HTTPS
argo profile install https://github.com/you/research-bot.git

# SSH
argo profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
argo profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
argo profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
argo profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
argo profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
argo profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Argo distribution.
...
```bash
cp ~/.argo/profiles/research-bot/.env.EXAMPLE ~/.argo/profiles/research-bot/.env
...
```bash
argo profile info research-bot
```
...
`argo profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
argo profile update research-bot
```
...
```bash
argo profile delete research-bot
```
...
Path:    ~/.argo/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `argo profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
argo profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `argo profile update pr-reviewer` and everyone's on the new version within minutes.
...
```bash
# You
cd ~/.argo/profiles/polymarket-trader
...
# Anyone
argo profile install github.com/you/argo-polymarket-trader --alias
```
...
You built Argo-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
argo profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
argo profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `argo profile info <name>` and hold off on updates until you're ready.
:::
...
argo profile info research-bot | grep Version
...
```yaml
# ~/.argo/profiles/research-bot/local/my-overrides.yaml
...
argo profile delete research-bot --yes
argo profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
argo profile update research-bot --force-config --yes
```
...
argo profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.argo/profiles/forked-research-bot/
...
argo profile install ~/.argo/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
argo profile delete research-bot-test --yes
argo profile install ~/.argo/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `argo -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `argo profile` — not a parallel command tree.
...
- Reserved profile names (`argo`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`argo profile export` / `import`](../reference/profile-commands.md#argo-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Argo](../guides/use-soul-with-argo.md) — authoring personalities
