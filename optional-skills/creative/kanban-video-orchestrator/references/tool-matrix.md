...
Maps each role archetype to the Argo skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public argo-agent
...
## Argo skills relevant to video production

### Visual / rendering skills (`argo-agent/skills/creative/`)
...
### Design / pre-production skills (`argo-agent/skills/creative/`)
...
### Audio / media skills (`argo-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`argo-agent/skills/devops/`)
...
These are **not** Argo skills but external CLIs / APIs that profiles invoke.
...
## Built-in Argo tools for media review

These are native Argo tools — not invoked via terminal but through their own
...
key is present in `~/.argo/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Argo profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.argo/.env` → environment variable.
...
When a new Argo-public video skill ships:
