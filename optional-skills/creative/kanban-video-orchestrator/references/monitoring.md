...
argo kanban watch --tenant <project-slug>

# Snapshot of the board
argo kanban list --tenant <project-slug>
argo kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
argo kanban stats --tenant <project-slug>

# Visual dashboard (browser)
argo dashboard

# Inspect a specific task (includes comments + events)
argo kanban show <task-id>

# Follow a single task's event stream
argo kanban tail <task-id>
```

Verify available subcommands with `argo kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `argo kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `argo kanban block <id>` then `argo kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `argo kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `argo kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `argo profile list` (profile exists?), `argo status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(argo kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
argo kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
argo kanban block <task-id>
argo kanban archive <task-id>

# Diagnose what happened
argo kanban show <task-id>      # task body, comments, recent events
argo kanban tail <task-id>      # follow the live event stream
argo kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `argo kanban create` for the director
...
while true; do
    clear
    argo kanban list --tenant <slug>
    echo "---"
    argo kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `argo kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `argo profile list` and start
