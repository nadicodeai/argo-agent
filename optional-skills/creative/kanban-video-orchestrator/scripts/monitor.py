#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `argo kanban list` and
...
def argo_available() -> bool:
    return shutil.which("argo") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed argo CLI."""
    try:
        out = subprocess.run(
            ["argo", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `argo kanban list`
    out = subprocess.run(
        ["argo", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not argo_available():
        print("ERROR: 'argo' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
