...
$ARGO_OSINT_CACHE/icij/ (default: ~/.cache/argo-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("ARGO_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "argo-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "argo-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $ARGO_OSINT_CACHE/icij or ~/.cache/argo-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
