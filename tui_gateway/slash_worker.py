"""Persistent slash-command worker — one ArgoCLI per TUI session.
...
def _run(cli: ArgoCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ARGO_SESSION_KEY"] = args.session_key
    os.environ["ARGO_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ArgoCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
