"""CLI entry point for the argo-agent ACP adapter.

Loads environment variables from ``~/.argo/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ARGO_HOME (default ``~/.argo``)."""
    from argo_cli.env_loader import load_argo_dotenv

    argo_home = get_argo_home()
    loaded = load_argo_dotenv(argo_home=argo_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", argo_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="argo-acp",
        description="Run Argo Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Argo version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import ArgoACPAgent  # noqa: F401

    print("Argo ACP check OK")


def _run_setup() -> None:
    from argo_cli.main import main as argo_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "argo", "model"]
        argo_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``argo postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting argo-agent ACP adapter")
    ...
    import acp
    from .server import ArgoACPAgent
