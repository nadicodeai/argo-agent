"""ACP auth helpers — detect and advertise Argo authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "argo-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Argo runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Argo can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Argo.
    ...
    not have Argo provider credentials configured yet, so Argo always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Argo using the currently configured "
