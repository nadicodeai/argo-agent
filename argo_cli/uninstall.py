"""
Argo Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.argo/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Argo PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing argo-agent or argo PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Argo Agent" comment and following line
                if '# Argo Agent' in line or '# argo-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('argo' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing argo
                if 'argo' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the argo wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "argo",
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains argo_cli reference)
                content = wrapper.read_text()
                if 'argo_cli' in content or 'argo-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``argo gateway run`` processes
    ...
    try:
        from argo_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from argo_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``ARGO_HOME`` and ``ARGO_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\argo\git\cmd``, ``%LOCALAPPDATA%\argo\git\bin``,
#      ``%LOCALAPPDATA%\argo\git\usr\bin``, ``%LOCALAPPDATA%\argo\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\argo\git\`` and Node to
#      ``%LOCALAPPDATA%\argo\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``argo dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\argo\gateway-service\`` and sometimes
...
def _argo_path_markers(argo_home: Path) -> list[str]:
    """Path-entry substrings that identify Argo-owned User-PATH entries."""
    root = str(argo_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare argo-agent install dir.
    markers = [root + "\\argo-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if ARGO_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "argo".  We don't want to catch
    # unrelated entries like "cargo-foo" or "ephermeral", so we look for
    # backslash-argo as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(argo_home: Path) -> list[str]:
    """Strip Argo-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _argo_path_markers(argo_home)
            kept: list[str] = []
...
def remove_argo_env_vars_windows() -> list[str]:
    """Delete ARGO_HOME and ARGO_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("ARGO_HOME", "ARGO_GIT_BASH_PATH"):
                try:
...
def _is_default_argo_home(argo_home: Path) -> bool:
    """Return True when ``argo_home`` points at the default (non-profile) root."""
    try:
        from argo_constants import get_default_argo_root
        return argo_home.resolve() == get_default_argo_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ARGO_HOME directory.

    We shell out to ``argo -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ARGO_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m argo_cli.main` so we don't depend on a `argo`
    #    wrapper that may be half-removed mid-uninstall.
    argo_invocation = [_sys.executable, "-m", "argo_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                argo_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ARGO_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.argo/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.argo/ for future reinstall
    """
    project_root = get_project_root()
    argo_home = get_argo_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ARGO_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_argo_home(argo_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Argo Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {argo_home / 'config.yaml'}")
    print(f"  Secrets: {argo_home / '.env'}")
    print(f"  Data:    {argo_home / 'cron/'}, {argo_home / 'sessions/'}, {argo_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ARGO_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Argo data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Argo code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in argo_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\argo\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(argo_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Argo-owned PATH entries in User environment")

        log_info("Removing ARGO_HOME / ARGO_GIT_BASH_PATH User env vars...")
        removed_env = remove_argo_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Argo-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing argo command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.argo/, just remove the argo-agent subdir
            if argo_home in project_root.parents or project_root.parent == argo_home:
    ...
    #     under ARGO_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(argo_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(argo_home)
    ...
    # 5. Optionally remove ~/.argo/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ARGO_HOME dirs live under
        ...
        try:
            if argo_home.exists():
                shutil.rmtree(argo_home)
                log_success(f"Removed {argo_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {argo_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {argo_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/argo-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/argo-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Argo Agent! ⚕")
    print()
