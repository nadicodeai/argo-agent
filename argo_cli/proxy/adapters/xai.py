...
# xAI's public API is OpenAI-compatible for the endpoints Argo commonly
# uses. The Responses endpoint is included because Argo' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Argo-managed OAuth credentials."""

    auth_hint = "argo auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`argo auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`argo auth reset xai-oauth` or re-authenticate with "
                    "`argo auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `argo auth add xai-oauth --type oauth`."
            )
