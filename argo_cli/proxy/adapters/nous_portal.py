...
Reads the user's Nous OAuth state from ``~/.argo/auth.json`` through the
...
:func:`argo_cli.auth.resolve_nous_runtime_credentials`.
"""
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(self, *, inference_auth_mode: str) -> UpstreamCredential:
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `argo login nous` first."
                )
            ...
            if not agent_key:
                raise RuntimeError(
                    ...
                    "Try `argo login nous` to re-authenticate."
                )
    ...
    # to argo_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
