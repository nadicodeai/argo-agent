"""CLI handlers for the ``argo proxy`` subcommand."""
...
from argo_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "argo proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"argo login {adapter.name}")
        print(
    ...
    print(
        f"Starting Argo proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Argo proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: argo proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``argo proxy <subcommand>``."""
    ...
    print(
        "argo proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  argo proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
