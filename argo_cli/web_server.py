"""
Argo Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m argo_cli.main web          # Start on http://127.0.0.1:9119
    python -m argo_cli.main web --port 8080
"""
...
try:
    ...
    # running `argo dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["ARGO_WEB_DIST"]) if "ARGO_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Argo Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-Argo-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``argo dashboard --tui``
# or ARGO_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.argo/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_argo_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "argo-update": "argo-update.log",
}
...
def _spawn_argo_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``argo <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``argo_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "argo_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ARGO_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``argo gateway restart`` in the background."""
    try:
        proc = _spawn_argo_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/argo/update")
async def update_argo():
    """Kick off ``argo update`` in the background."""
    try:
        proc = _spawn_argo_action(["update"], "argo-update")
    except Exception as exc:
        _log.exception("Failed to spawn argo update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Argo supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in argo_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.argo/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``argo auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Argo resolves Anthropic creds in this order at runtime:
    1. ``~/.argo/.anthropic_oauth.json`` — Argo-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_argo_oauth_credentials,
            read_claude_code_credentials,
            _ARGO_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_argo_oauth_credentials = None  # type: ignore
        _ARGO_OAUTH_FILE = None  # type: ignore

    argo_creds = None
    if read_argo_oauth_credentials:
        try:
            argo_creds = read_argo_oauth_credentials()
        except Exception:
            argo_creds = None
    if argo_creds and argo_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "argo_pkce",
            "source_label": f"Argo PKCE ({_ARGO_OAUTH_FILE})",
            "token_preview": _truncate_token(argo_creds.get("accessToken")),
            "expires_at": argo_creds.get("expiresAt"),
            "has_refresh_token": bool(argo_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Argo even
    when they also have a separate Argo-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "argo auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "argo auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "argo auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "argo auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "argo auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from argo_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("argo_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Argo-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _ARGO_OAUTH_FILE
            if _ARGO_OAUTH_FILE.exists():
                _ARGO_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from argo_cli.auth import clear_provider_auth
...
#          → persists to ~/.argo/.anthropic_oauth.json AND credential pool
...
# Guarded so argo web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Argo file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``argo auth add anthropic``.
    """
    from agent.anthropic_adapter import _ARGO_OAUTH_FILE
    payload = {
    ...
    _ARGO_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _ARGO_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("ARGO_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from argo_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from argo_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_argo_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from argo_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from argo_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, ARGO_HOME)."""
    from argo_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process ARGO_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_argo_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "argo setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from argo_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from argo_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from argo_cli.tools_config import (
...
#
# The endpoint spawns the same ``argo --tui`` binary the CLI uses, behind
...
try:
    from argo_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``argo --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ARGO_TUI_RESUME`` env var —
    matching what ``argo_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``ARGO_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from argo_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("ARGO_TUI_DISABLE_MOUSE", "1")
    env.setdefault("ARGO_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["ARGO_TUI_RESUME"] = resume

    if sidecar_url:
        env["ARGO_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Argo inside WSL2 to use the dashboard's /chat "
...
# ARGO_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/argo"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/argo/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /argo`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__ARGO_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/argo``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__ARGO_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__ARGO_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__ARGO_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/argo`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Argo backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Argo Teal",         "description": "Classic dark teal — the canonical Argo look"},
    {"name": "default-large", "label": "Argo Teal (Large)", "description": "Argo Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.argo/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.argo/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_argo_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.argo/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as argo_cli.plugins):
    1. User plugins:    ~/.argo/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.argo/plugins/  (only if ARGO_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from argo_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_argo_home() / "plugins", "user"),
    ...
    # ``argo_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("ARGO_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".argo" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from argo_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_argo_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from argo_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from argo_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from argo_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from argo_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.argo/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"argo_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  Argo Web UI → http://{host}:{port}")
