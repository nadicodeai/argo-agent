...
Pure logic: walks a Argo config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `argo doctor` and a future `argo migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Argo config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Argo config and return retirement issues.
