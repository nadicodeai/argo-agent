...
Usage:
    argo pairing list              # Show all pending + approved users
    argo pairing approve <platform> <code>  # Approve a pairing code
    argo pairing revoke <platform> <user_id> # Revoke user access
    argo pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle argo pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: argo pairing {list|approve|revoke|clear-pending}")
        print("Run 'argo pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.argo/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'argo pairing list' to see pending codes.\n")
