#!/usr/bin/env python3
"""
Argo CLI - Main entry point.

Usage:
    argo                     # Interactive chat (default)
    argo chat                # Interactive chat
    argo gateway             # Run gateway in foreground
    argo gateway start       # Start gateway as service
    argo gateway stop        # Stop gateway service
    argo gateway status      # Show gateway status
    argo gateway install     # Install gateway service
    argo gateway uninstall   # Uninstall gateway service
    argo setup               # Interactive setup wizard
    argo logout              # Clear stored authentication
    argo status              # Show status of all components
    argo cron                # Manage cron jobs
    argo cron list           # List cron jobs
    argo cron status         # Check if cron scheduler is running
    argo doctor              # Check configuration and dependencies
    argo honcho setup                    # Configure Honcho AI memory integration
    argo honcho status                   # Show Honcho config and connection status
    argo honcho sessions                 # List directory → session name mappings
    argo honcho map <name>               # Map current directory to a session name
    argo honcho peer                     # Show peer names and dialectic settings
    argo honcho peer --user NAME         # Set user peer name
    argo honcho peer --ai NAME           # Set AI peer name
    argo honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    argo honcho mode                     # Show current memory mode
    argo honcho mode [hybrid|honcho|local]  # Set memory mode
    argo honcho tokens                   # Show token budget settings
    argo honcho tokens --context N       # Set session.context() token cap
    argo honcho tokens --dialectic N     # Set dialectic result char cap
    argo honcho identity                 # Show AI peer identity representation
    argo honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    argo honcho migrate                  # Step-by-step migration guide: OpenClaw native → Argo + Honcho
    argo version             Show version
    argo update              Update to latest version
    argo uninstall           Uninstall Argo Agent
    argo acp                 Run as an ACP server for editor integration
    argo sessions browse     Interactive session picker with search

    argo claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: argo_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``argo_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``argo update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``argo_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, argo crashes on import and the user can't run
# ``argo update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import argo_bootstrap  # noqa: F401
...
def _print_fast_version_info() -> None:
    from argo_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Argo Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``argo --version`` before config/logging imports on Termux."""
    if os.environ.get("ARGO_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ARGO_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (argo tools, argo setup, argo model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'argo {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any argo module import.
#
# Many modules cache ARGO_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ARGO_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.argo/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ARGO_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors argo_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If ARGO_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.argo/profiles/coder or
    # /opt/data/profiles/coder).  If ARGO_HOME points to the argo root
    # instead (e.g. systemd hardcodes ARGO_HOME=/root/.argo), we must
    # still read active_profile — the user may have switched profiles via
    # `argo profile use` and the gateway should honour that choice.
    # See issue #22502.
    argo_home_env = os.environ.get("ARGO_HOME", "")
    if profile_name is None and argo_home_env:
        if Path(argo_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the argo root
    if profile_name is None:
        try:
            from argo_constants import get_default_argo_root

            active_path = get_default_argo_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ARGO_HOME
    if profile_name is not None:
        try:
            from argo_cli.profiles import resolve_profile_env

            argo_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent argo from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ARGO_HOME"] = argo_home
...
# Load .env from ~/.argo/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from argo_cli.config import get_argo_home
from argo_cli.env_loader import load_argo_dotenv

load_argo_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ARGO_REDACT_SECRETS env
# var BEFORE argo_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "ARGO_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_argo_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["ARGO_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `argo` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from argo_logging import setup_logging as _setup_logging
...
try:
    from argo_cli.config import load_config as _load_config_early
    from argo_constants import apply_ipv4_preference as _apply_ipv4
...
from argo_cli import __version__, __release_date__
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_argo_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("ARGO_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("ARGO_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from argo_cli.config import get_env_path, get_argo_home, load_config
    from argo_cli.auth import get_auth_status

    # Determine whether Argo itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from argo_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_argo_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from argo_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_argo_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Argo has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Argo to use their tokens.
    if _has_argo_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, argo_bin
        cli_args: the original CLI arguments (everything after 'argo')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    argo_bin = container_info["argo_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo argo {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo argo {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  argo --tui --resume {target}")
    if title:
        print(f'  argo --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @argo/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@argo" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/argo-ink/package.json",
    "packages/argo-ink/package-lock.json",
    "packages/argo-ink/index.js",
    "packages/argo-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``ARGO_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("ARGO_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``ARGO_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ARGO_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    argo_home = os.environ.get("ARGO_HOME") or str(Path.home() / ".argo")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ARGO_HOME": argo_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(argo_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(argo_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if argo_cli_dir is None:
        argo_cli_dir = Path(__file__).parent
    bundled = argo_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ARGO_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ARGO_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from argo_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("ARGO_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with ARGO_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset ARGO_TUI_DIR (e.g. `unset ARGO_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("ARGO_QUIET"):
    ...
    if tui_dev:
        # Keep the local @argo/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @argo/ink resolves through
        # packages/argo-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "argo-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from argo_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="argo-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ARGO_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ARGO_PYTHON_SRC_ROOT"] = os.environ.get(
        "ARGO_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ARGO_PYTHON", sys.executable)
    env.setdefault("ARGO_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["ARGO_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["ARGO_MODEL"] = model
        env["ARGO_INFERENCE_MODEL"] = model
    if provider:
        env["ARGO_TUI_PROVIDER"] = provider
        env["ARGO_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ARGO_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["ARGO_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["ARGO_TUI_SKILLS"] = value
    if query:
        env["ARGO_TUI_QUERY"] = query
    if image:
        env["ARGO_TUI_IMAGE"] = image
    if checkpoints:
        env["ARGO_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["ARGO_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["ARGO_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["ARGO_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["ARGO_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
    ...
    # ARGO_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `argo --tui` try to
    ...
    # still set ARGO_TUI_RESUME themselves.
    env.pop("ARGO_TUI_RESUME", None)
    if resume_session_id:
        env["ARGO_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `argo update` so
    ...
    if code == 42:
        from argo_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``ARGO_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``argo kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``argo kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("ARGO_KANBAN_BOARD"):
        return
    try:
        from argo_cli.kanban_db import get_current_board

        os.environ["ARGO_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("ARGO_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'argo sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from argo_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from argo_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'argo doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Argo isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  argo setup")
        print()

        from argo_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'argo setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from argo_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ARGO_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.argo/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ARGO_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ARGO_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ARGO_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from argo_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from argo_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from argo_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Argo?")
        print()
    ...
    # has no creds.json.  Every subsequent `argo gateway` then paid a 30s
    ...
    session_dir = get_argo_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: argo gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `argo gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  argo gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Argo Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  argo gateway")
        ...
        print()
        print("  Or install as a service: argo gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'argo whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from argo_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from argo_cli.config import stamp_install_method
    from argo_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Argo post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``argo model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from argo_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from argo_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from argo_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ARGO_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from argo_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'argo model' for "
                "available providers, or run 'argo doctor' to diagnose config "
                "issues."
            )
    ...
    from argo_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    # a leftover OPENAI_BASE_URL in ~/.argo/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.argo/.env if the active provider is not 'custom'.
    ...
    """
    from argo_cli.config import get_env_value, save_env_value, load_config
...
#
# Argo uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `argo model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `argo model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from argo_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from argo_cli.config import load_config
    ...
    while True:
        ...
        print("  Argo only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``argo model`` flow, then route aux tasks to them here.
    """
    from argo_cli.config import load_config
    from argo_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from argo_cli.auth import _prompt_model_selection
    from argo_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from argo_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from argo_constants import OPENROUTER_BASE_URL
    from argo_cli.auth import (
        ProviderConfig,
    ...
    from argo_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.argo/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `argo setup` from scratch. OpenRouter
    ...
    from argo_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from argo_constants import AI_GATEWAY_BASE_URL
    from argo_cli.auth import (
        PROVIDER_REGISTRY,
        _prompt_model_selection,
        _save_model_choice,
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.argo/.env by hand.
    ...
    from argo_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from argo_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from argo_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from argo_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `argo auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from argo_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from argo_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``argo model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``argo model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``argo auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from argo_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from argo_cli.auth import _save_model_choice, deactivate_provider
    from argo_cli.config import get_env_value, load_config, save_config
    ...
    from argo_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `argo model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from argo_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Argo URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from argo_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from argo_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from argo_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Argo will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  Argo will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from argo_cli.curses_ui import flush_stdin
...
# it to the model-selection handlers so plain `argo --tui` does not pay for
...
def _model_flow_copilot(config, current_model=""):
    ...
    from argo_cli.config import save_env_value, load_config, save_config
    from argo_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from argo_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from argo_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from argo_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Argo turns to `copilot --acp`.")
    print("  Argo currently starts its own ACP subprocess for each request.")
    print("  Argo uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ARGO_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``argo setup`` / ``argo model``.
    ...
    recover from a malformed paste without editing ``~/.argo/.env`` by hand.
    ...
    from argo_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from argo_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `argo setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from argo_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so argo knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Argo typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Argo, enable billing on your "
    ...
    if provider_id == "lmstudio":
        ...
        from argo_cli.models import fetch_lmstudio_models
        ...
        from argo_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from argo_constants import display_argo_home as _dhh_fn

            print(
                f"    Argo will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from argo_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'argo model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Argo CLI with a provider."""
    from argo_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from argo_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from argo_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from argo_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from argo_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from argo_cli.webhook import webhook_command
...
def cmd_portal(args):
    ...
    from argo_cli.portal_cli import portal_command
...
def cmd_slack(args):
    ...
    Dispatches ``argo slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: argo slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `argo slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from argo_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from argo_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from argo_cli.hooks import hooks_command
...
def cmd_dump(args):
    ...
    from argo_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from argo_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from argo_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Argo home directory to a zip file."""
    if getattr(args, "quick", False):
        from argo_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from argo_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Argo backup from a zip file."""
    from argo_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from argo_cli.banner import check_for_updates
        from argo_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from argo_cli.uninstall import run_uninstall
...
# Critical files that every ``argo`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``argo update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "argo_cli/main.py",
    "argo_cli/config.py",
    "argo_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``argo`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="argo-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``argo update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``argo_cli/web_dist/`` (per vite.config.ts
    outDir: "../argo_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "argo_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``argo update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``argo web``).
    ...
    # build path usable on a stock `py -m argo_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (argo web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``argo dashboard`` processes other than ourselves.

    ``argo dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``argo update`` replaces files on disk, the running
    ...
    patterns = [
        "argo dashboard",
        "argo_cli.main dashboard",
        "argo_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "argo.*dashboard"`) keeps us consistent
            # with `argo_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `argo update`.
    ...
    print("  Preview now:  argo curator run --dry-run")
    print("  Pause it:     argo curator pause")
    print(
        "  Docs:         https://argo-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``argo update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``argo update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: argo curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``argo dashboard`` processes.

    Called at the end of ``argo update`` (default ``reason``) and also
    from ``argo dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    argo dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Argo Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/argo-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="argo-update-")
        zip_path = os.path.join(tmp_dir, f"argo-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to argo-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"argo-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "argo-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Argo behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes argo completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Argo couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Argo couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Argo behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `argo update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/argo-agent.git",
    "git@github.com:NousResearch/argo-agent.git",
    "https://github.com/NousResearch/argo-agent",
    "git@github.com:NousResearch/argo-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/argo-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_argo_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_argo_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Argo repository.")
        print("  This means you may miss updates from NousResearch/argo-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/argo-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/argo-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``argo update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from argo_constants import get_default_argo_root

    default_home = get_default_argo_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``argo update`` is still progressing even if pip/uv itself is silent.
    """
...
def _argo_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "argo.exe",
        scripts_dir / "argo-gateway.exe",
    ]


def _detect_concurrent_argo_instances(
    ...
    The Argo Desktop Electron app spawns ``argo.EXE`` as a backend child,
    so during ``argo update`` the user-invoked process and the desktop's
    ...
    shims (``argo.exe`` / ``argo-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID is excluded so the running
    ``argo update`` invocation never reports itself.
    ...
    for shim in _argo_exe_shims(scripts_dir):
        try:
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "argo.exe"
    lines = ["✗ Another argo.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Argo Desktop, exit any open `argo` REPLs, and")
    lines.append("  stop the gateway (`argo gateway stop`) before retrying.")
    lines.append("  Override with `argo update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_argo_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``argo.exe``.
    ...
    when ``argo update`` runs, ``argo.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``argo.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next argo invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Argo Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Argo Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _argo_exe_shims(scripts_dir):
        ...
        print(
            "    Close Argo Desktop, exit other `argo` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `argo update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_argo_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``argo.exe.old.*`` left by prior updates.

    Called early on every argo invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``argo update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `argo update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``argo.exe`` / ``argo-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_argo_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_argo_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.argo/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``argo update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.argo/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``argo update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``argo update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.argo/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``argo update --gateway``) the update is already
    ...
    try:
        ...
        # argo_cli.config.get_argo_home to simulate setup failure.
        from argo_cli.config import get_argo_home as _get_argo_home

        logs_dir = _get_argo_home() / "logs"
...
def _cmd_update_check():
    """Implement ``argo update --check``: fetch and report without installing."""
    from argo_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from argo_cli.config import recommended_update_command
        from argo_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``argo update`` without requiring a reinstall.  The
    ...
    ``argo`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/argo.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v argo'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/argo, code at /usr/local/lib/argo-agent).
    fhs_link = Path("/usr/local/bin/argo")
    ...
    path_comment = (
        "# Argo Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ARGO_HOME before running the update.
    ...
    ARGO_HOME directories.  The ``--backup`` flag on ``argo update``
    ...
    try:
        from argo_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_argo_home so the user sees ~/.argo/...
    try:
        from argo_constants import get_argo_home, display_argo_home

        home = get_argo_home()
        try:
            display_path = f"{display_argo_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  argo import {out_path}")
...
def cmd_update(args):
    """Update Argo Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from argo_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Argo Agent")
        return
...
def _cmd_update_pip(args):
    """Update Argo via pip (for PyPI installs)."""
    from argo_cli import __version__
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "argo-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "argo-agent"]
    ...
    print("✓ Update complete! Restart argo to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Argo Agent...")
    print()

    # On Windows, abort early if another argo.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_argo_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # orphan merge-conflict markers in argo_cli/config.py bricked
        # every user who ran ``argo update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``argo
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``argo update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_argo_home added to argo_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload argo_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_argo_home() added since the last release.
        try:
            import importlib
            import argo_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit ARGO_HOME so
        # it is not affected by sync_skills()'s module-level ARGO_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's ARGO_HOME env var points at the default or a named profile.
        try:
            from argo_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: argo config migrate")
            else:
                print()
                print("Skipped. Run 'argo config migrate' later to configure.")
        else:
        ...
        # check `argo curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``argo update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from argo_cli.tools_config import install_cua_driver
        ...
        # When running as ``argo update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_argo_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all argo-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. argo-gateway.service or argo-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: argo gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: argo -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Argo gateway unit files are still installed.
        # When both argo.service (from a pre-rename install) and the
        # current argo-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `argo update` surfaces the issue until the user migrates.
        try:
            from argo_cli.gateway import (
                has_legacy_argo_units,
                _find_legacy_argo_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_argo_units():
                print()
                print("⚠ Legacy Argo gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_argo_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (argo.service) fight the current")
                print("  argo-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    argo gateway migrate-legacy")
                print()
        ...
        print("  argo model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``argo -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from argo_constants import display_argo_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `argo profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_argo_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → argo -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.argo)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  argo profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     argo -p {name} chat")
                else:
        ...
        from argo_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from argo_constants import get_argo_home as _hh
        ...
        from argo_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `argo profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from argo_cli.profiles import profile_exists
        ...
        from argo_cli.profiles import rename_profile
        ...
        from argo_cli.profiles import export_profile
        ...
        from argo_cli.profiles import import_profile
        ...
        import tempfile
        from argo_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="argo_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  argo -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      argo -p {plan.manifest.name} chat")
        ...
        from argo_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `argo profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"argo -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from argo_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("argo_requires"):
            print(f"Requires:     Argo {data['argo_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from argo_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.argo_requires:
        print(f"  Requires: Argo {mf.argo_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``argo dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``argo dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No argo dashboard processes running.")
        return 0

    print(f"{len(pids)} argo dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No argo dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `argo update`.
    ...
    if "ARGO_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["ARGO_WEB_DIST"])
            if "ARGO_WEB_DIST" in os.environ
            else PROJECT_ROOT / "argo_cli" / "web_dist"
        )
    ...
    from argo_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("ARGO_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from argo_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Argo log files."""
    from argo_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from argo_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``argo -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``argo_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``argo -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``argo`` or only flags → defaults to ``chat``.
        return False
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        from argo_cli.plugins import discover_plugins
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    if "-h" in argv or "--help" in argv:
        return False
    if os.environ.get("ARGO_TUI") == "1" or "--tui" in argv:
        return False
    ...
    from argo_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from argo_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["ARGO_DEFER_AGENT_STARTUP"] = "1"
            os.environ["ARGO_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["ARGO_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `argo --tui` is the hot path on phones. The full parser setup imports
    ...
    wants_tui = os.environ.get("ARGO_TUI") == "1" or "--tui" in sys.argv[1:]
    if not wants_tui:
        return False
    ...
    if getattr(args, "command", None) not in {None, "chat"}:
        return False
    if not (getattr(args, "tui", False) or os.environ.get("ARGO_TUI") == "1"):
        return False
...
def main():
    """Main entry point for argo CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from argo_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``argo.exe.old.*`` quarantine files left by previous
    # ``argo update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_argo_exe``.
    try:
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: argo-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from argo_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://argo-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `argo model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.argo/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://argo-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from argo_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from argo_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy argo.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Argo gateway unit files "
            "(e.g. argo.service) left over from older installs. Profile "
            "units (argo-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `argo proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Argo Agent with an interactive wizard. "
        "Run a specific section: argo setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'argo setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Argo.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $ARGO_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Argo")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from argo_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Argo CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: argo-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Argo with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set ARGO_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Argo Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.argo/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="Argo profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.argo/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="Argo profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from argo_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from argo_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.argo/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.argo/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Argo Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Argo setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Argo Agent. Use 'argo debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    argo debug share              Upload debug report and print URL
    argo debug share --lines 500  Include more log lines
    argo debug share --expire 30  Keep paste for 30 days
    argo debug share --local      Print report locally (no upload)
    argo debug share --no-redact  Disable upload-time secret redaction
    argo debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'argo debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Argo home directory to a zip file",
        description="Create a zip archive of your entire Argo configuration, "
        "skills, sessions, and data (excludes the argo-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/argo-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.argo/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo argo uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from argo_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Argo backup from a zip file",
        description="Extract a previously created Argo backup into your "
        "Argo home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Argo Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from argo_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.argo/skills/.bundled_manifest) "
            "so future 'argo update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from argo_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from argo_cli.skills_hub import skills_command
    ...
    from argo_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/argo-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `argo plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from argo_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``argo --help``, ``argo version``, ``argo logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from argo_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from argo_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_argo_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_argo_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_argo_home()}/memories/\n")
        else:
            from argo_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'argo tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # argo tools disable <name...> [--platform cli]
    ...
    # argo tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from argo_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from argo_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `argo computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `argo tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: argo computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: argo computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Argo as an MCP server",
        description=(
            "Manage MCP server connections and run Argo as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'argo mcp add' to connect to a new server, or\n"
            "'argo mcp serve' to expose Argo conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Argo as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `argo mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from argo_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_argo_home() / "sessions"
            ...
            # Launch argo --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Argo",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Argo",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.argo/ (by default a "
        "single restore-point archive is written to ~/.argo/backups/ "
        "before apply; restorable with 'argo import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from argo_cli.claw import claw_command
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Argo Agent",
        description="Remove Argo Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Argo Agent as an ACP (Agent Client Protocol) server",
        description="Start Argo Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Argo ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Argo provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.argo/node/ "
    ...
    def cmd_acp(args):
        """Launch Argo Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Argo instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `argo update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `argo profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Argo profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Argo Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `argo --tui` via PTY/WebSocket). "
            "Alternatively set ARGO_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `argo dashboard` cmdlines and SIGTERM them directly — the same
    # path `argo update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running argo dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running argo dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Argo log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    argo logs                    Show last 50 lines of agent.log
    argo logs -f                 Follow agent.log in real time
    argo logs errors             Show last 50 lines of errors.log
    argo logs gateway -n 100     Show last 100 lines of gateway.log
    argo logs --level WARNING    Only show WARNING and above
    argo logs --session abc123   Filter by session ID
    argo logs --component tools  Only show tool-related lines
    argo logs --since 1h         Lines from the last hour
    argo logs --since 30m -f     Follow, starting from 30 min ago
    argo logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``argo -c Pokemon Agent Dev`` → ``argo -c 'Pokemon Agent Dev'``
    ...
    from argo_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'argo -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (argo hooks list, cron
