"""
Dump command for argo CLI.

Outputs a compact, plain-text summary of the user's Argo setup
...
from argo_cli.config import get_argo_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``argo dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from argo_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(argo_home: Path) -> int:
...
def _cron_summary(argo_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = argo_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"argo_home:      {display_argo_home()}")
    ...
    toolsets = config.get("toolsets", ["argo-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(argo_home)}")
    lines.append(f"  skills:             {_count_skills(argo_home)}")
