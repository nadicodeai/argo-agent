"""Shell completion script generation for argo CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_argo_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Argo Agent bash completion
# Add to ~/.bashrc:
#   eval "$(argo completion bash)"

_argo_profiles() {{
    local profiles_dir="$HOME/.argo/profiles"
...
_argo_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_argo_profiles)" -- "$cur"))
        return
    fi
...
complete -F _argo_completion argo
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef argo
# Argo Agent zsh completion
# Add to ~/.zshrc:
...
_argo_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.argo/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.argo/profiles 2>/dev/null)}}")
    fi
...
_argo() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_argo_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'argo command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Argo Agent fish completion",
        "# Add to your config:",
        "#   argo completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __argo_profiles",
        "    echo default",
        "    if test -d $HOME/.argo/profiles",
        "        ls $HOME/.argo/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c argo -f -s p -l profile"
        " -d 'Profile name' -xa '(__argo_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__argo_profiles)' -d 'Profile name'"
                )
