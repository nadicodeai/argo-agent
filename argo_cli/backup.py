"""
Backup and import commands for argo CLI.

`argo backup` creates a zip archive of the entire ~/.argo/ directory
(excluding the argo-agent repo and transient files).

`argo import` restores from a backup zip, overlaying onto the current
ARGO_HOME root.
"""
...
from argo_constants import get_default_argo_root, get_argo_home, display_argo_home
...
_EXCLUDED_DIRS = {
    "argo-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to argo root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Argo home directory."""
    argo_root = get_default_argo_root()

    if not argo_root.is_dir():
        print(f"Error: Argo home directory not found at {argo_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"argo-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"argo-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_argo_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(argo_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(argo_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(argo_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside argo root
            try:
    ...
    print(f"\nRestore with: argo import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Argo backup.
    ...
    # Look for telltale files that a argo home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Argo backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.argo/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a argo dir name
        if prefix in {".argo", "argo"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_argo_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (argo_root / "config.yaml").exists()
        has_env = (argo_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Argo configuration.")
        ...
        for member in members:
            ...
            target = argo_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(argo_root.resolve())
            except ValueError:
        ...
        profiles_dir = argo_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # argo_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: argo profile list  (after installing argo)")

        # Guidance
        print()
        if not (argo_root / "argo-agent").is_dir():
            print("Note: The argo-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: argo update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  argo -p {pname} gateway install")

        print("Done. Your Argo configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and argo backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ARGO_HOME).
...
# — `argo update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(argo_home: Optional[Path] = None) -> Path:
    home = argo_home or get_argo_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    argo_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(argo_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(argo_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for argo backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_argo_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, argo_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``argo_root`` to ``out_path``.
...
def _pre_update_backup_dir(argo_home: Optional[Path] = None) -> Path:
    home = argo_home or get_argo_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ARGO_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ARGO_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``argo update``) should continue even if the backup fails.
    """
    argo_root = argo_home or get_default_argo_root()
    if not argo_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(argo_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, argo_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `argo claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ARGO_HOME under ``backups/`` before a
    ``argo claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``argo import <archive>``.  Writes to
    ``<ARGO_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `argo import` and the
