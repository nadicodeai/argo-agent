"""
Status command for argo CLI.

Shows the status of all Argo Agent components.
"""
...
from argo_cli.auth import AuthError, resolve_provider
from argo_cli.colors import Colors, color
from argo_cli.config import get_env_path, get_env_value, get_argo_home, load_config
from argo_cli.models import provider_label
from argo_cli.nous_subscription import get_nous_subscription_features
from argo_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``argo config``'s
...
def show_status(args):
    """Show status of all Argo Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Argo Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: argo auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: argo model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: argo auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: argo auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: argo model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from argo_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'argo-agent[vercel]')"
    ...
    try:
        from argo_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_argo_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_argo_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'argo doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'argo setup' to configure", Colors.DIM))
    print()
