"""CLI handlers for ``argo migrate ...``.

Currently exposes only ``argo migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``argo migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: argo migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `argo migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `argo doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
