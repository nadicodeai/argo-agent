"""``argo debug`` debug tools for Argo Agent.

Currently supports:
    argo debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.argo/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[argo debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.argo/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``argo debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``argo debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Argo version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `argo debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "argo-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``argo debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.argo/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``argo debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"argo debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "argo-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ArgoDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from argo_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_argo_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``argo dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``argo dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "argo debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  argo debug delete <url>")

    print(f"\nShare these links with the Argo team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: argo debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'argo debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``argo debug`` call.
    ...
    # best-effort — any failure is swallowed so ``argo debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: argo debug <command>")
        print()
        print("Commands:")
