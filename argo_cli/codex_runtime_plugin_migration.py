"""Migrate Argo' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Argo' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Argo mcp_servers.<n>.command/args/env  → codex stdio transport
  Argo mcp_servers.<n>.url/headers       → codex streamable_http transport
  Argo mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Argo mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Argo-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Argo' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Argo config.")
...
# Argo keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Argo' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, argo_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Argo MCP server config to the codex inline-table dict
    ...
    if not isinstance(argo_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(argo_cfg.get("command"))
    has_url = bool(argo_cfg.get("url"))
    ...
    for key in argo_cfg:
        ...
        elif key not in _KNOWN_ARGO_KEYS:
            skipped.append(f"{key} (unknown Argo key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ARGO_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Argo)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Argo' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Argo' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Argo
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="argo-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ARGO_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed argo-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.argo`` symlink that happens to live under ``/private/var/folders``
...
def _build_argo_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Argo' own
    ...
    so a argo installed under /opt/, /usr/local/, or a venv all work.
    ARGO_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ARGO_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_argo_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever ARGO_HOME its launcher sets
    ...
    # — that would override the launcher's ARGO_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ARGO_HOME", tmp_path) would
    ...
    argo_home = os.environ.get("ARGO_HOME") or ""
    if argo_home and _looks_like_test_tempdir(argo_home):
        argo_home = ""
    if argo_home:
        env["ARGO_HOME"] = argo_home
    # PYTHONPATH passes through so a worktree-launched argo finds the
    ...
    env["ARGO_QUIET"] = "1"
    env["ARGO_REDACT_SECRETS"] = env.get("ARGO_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_argo_tools: bool = True,
) -> MigrationReport:
    """Translate Argo mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        argo_config: full ~/.argo/config.yaml dict
        ...
        expose_argo_tools: when True (default), register Argo' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Argo for tools
    ...
    argo_servers = (argo_config or {}).get("mcp_servers") or {}
    if not isinstance(argo_servers, dict):
        report.errors.append(
            "mcp_servers in Argo config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in argo_servers.items():
    ...
    # Inject Argo' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Argo for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/argo_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_argo_tools:
        translated["argo-tools"] = _build_argo_tools_mcp_entry()
        if "argo-tools" not in report.migrated:
            report.migrated.append("argo-tools")
