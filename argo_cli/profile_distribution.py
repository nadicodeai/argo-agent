"""Profile distributions — shareable, packaged Argo profiles via git.

A distribution is a Argo profile published as a git repository (or
...
* ``argo profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``argo skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``argo profile``, not a parallel tree):

    argo profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "argo-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            argo_requires=str(data.get("argo_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.argo_requires:
            out["argo_requires"] = self.argo_requires
        if self.author:
...
def check_argo_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Argo {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``argo profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Argo profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Argo distribution."
        )

    # Version check up-front so we fail fast
    check_argo_requires(manifest.argo_requires, argo_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.argo).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="argo_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `argo profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`argo profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="argo_dist_update_") as tmp:
        plan = plan_install(
