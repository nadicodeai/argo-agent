"""``argo logs`` — view and filter Argo log files.
...
Usage examples::
    ...
    argo logs                    # last 50 lines of agent.log
    argo logs -f                 # follow agent.log in real time
    argo logs errors             # last 50 lines of errors.log
    argo logs gateway -n 100    # last 100 lines of gateway.log
    argo logs --level WARNING    # only WARNING+ lines
    argo logs --session abc123   # filter by session ID substring
    argo logs --component tools  # only tool-related lines
    argo logs --since 1h         # lines from the last hour
    argo logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_argo_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Argo runs — try 'argo chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_argo_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_argo_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_argo_home()}/logs/")
        return

    print(f"Log files in {display_argo_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'argo chat' to generate logs)")
