"""Slash command definitions and autocomplete for the Argo CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Argo config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Argo works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Argo is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.argo/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Argo Agent to the latest version", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`argo_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/argo`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Argo
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``argo skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``argo skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``argo skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/argo <verb>`` subcommand.
    ...
    can still reach them via ``/argo <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/argo`` is always reserved as the first entry so the
    legacy ``/argo <subcommand>`` form keeps working for anything that
    ...
    # Reserve /argo as the catch-all top-level command.
    entries.append(("argo", "Talk to Argo or run a subcommand", "[subcommand] [args]"))
    seen.add("argo")
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /argo handler.

    Maps both canonical names and aliases so /argo bg do stuff works
    the same as /argo background do stuff.

    Plugin-registered slash commands are included so ``/argo <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from argo_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
