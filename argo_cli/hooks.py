"""argo hooks — inspect and manage shell-script hooks.

Usage::

    argo hooks list
    argo hooks test <event> [--for-tool X] [--payload-file F]
    argo hooks revoke <command>
    argo hooks doctor

Consent records live under ``~/.argo/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.argo/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``argo hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: argo hooks {list|test|revoke|doctor}")
        print("Run 'argo hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.argo/config.yaml.")
        print("See `argo hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `argo hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `argo hooks test` and `argo hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Argo wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `argo hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `argo hooks doctor` would execute
