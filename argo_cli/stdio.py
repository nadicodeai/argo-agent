...
on Japanese locales, etc.).  Argo's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``argo_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Argo is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ARGO_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ARGO_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Argo-managed Git install directories so
    ...
    # already-running shells don't see the change — which means argo
    ...
    # install without Argo-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Argo's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Argo's
    ``argo config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Argo (User env var in Windows Settings, or
    export in a PowerShell profile) and Argo picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Argo-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\argo\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``argo`` from right after install) retain their old PATH.

    Any subprocess Argo spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Argo-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "argo", "git", "cmd"),
        os.path.join(local_appdata, "argo", "git", "bin"),
        os.path.join(local_appdata, "argo", "git", "usr", "bin"),
        # Argo venv Scripts directory — host of the argo.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes argo, but harmless to include.
        os.path.join(local_appdata, "argo", "argo-agent", "venv", "Scripts"),
