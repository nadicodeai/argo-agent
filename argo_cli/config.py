"""
Configuration management for Argo Agent.

Config files are stored in ~/.argo/ for easy access:
- ~/.argo/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.argo/.env         - API keys and secrets

This module provides:
- argo config          - Show current configuration
- argo config edit     - Open config in editor
- argo config set      - Set a specific value
- argo config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.argo/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``argo logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  argo config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `argo plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "ARGO_LANGFUSE_ENV",
    "ARGO_LANGFUSE_RELEASE",
    "ARGO_LANGFUSE_SAMPLE_RATE",
    "ARGO_LANGFUSE_MAX_CHARS",
    "ARGO_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from argo_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ARGO_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_argo_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Argo is running in package-manager-managed mode.

    Two signals: the ARGO_MANAGED env var (set by the systemd service),
    or a .managed marker file in ARGO_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade argo-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Argo was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.argo/.install_method`` file (written by installers)
    2. ARGO_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_argo_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.argo/.install_method."""
    stamp = get_argo_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade argo-agent"
    if method == "docker":
        return "docker pull nousresearch/argo-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade argo-agent"
        return "pip install --upgrade argo-agent"
    return "argo update"
...
def format_managed_message(action: str = "modify this Argo installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ARGO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Argo installation is managed by NixOS "
            f"(ARGO_MANAGED={env_hint}).\n"
            "Edit services.argo-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Argo installation is managed by Homebrew "
            f"(ARGO_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade argo-agent"
        )

    return (
        f"Cannot {action}: this Argo installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Argo."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ARGO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, argo_bin
    or None if container mode is not active, we're already inside the
    container, or ARGO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ARGO_DEV") == "1":
        return None
    ...
    container_mode_file = get_argo_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "argo-agent")
    exec_user = info.get("exec_user", "argo")
    argo_bin = info.get("argo_bin", "/data/current-package/bin/argo")
...
# Re-export from argo_constants — canonical definition lives there.
from argo_constants import get_argo_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_argo_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_argo_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the argo group can
    share state with the gateway service.

    The mode can be overridden via the ARGO_HOME_MODE environment variable
    (e.g. ARGO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ARGO_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ARGO_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Argo runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ARGO_CONTAINER") or os.environ.get("ARGO_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ARGO_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ARGO_HOME if the user doesn't have one yet."""
...
def ensure_argo_home():
    """Ensure ~/.argo directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_argo_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_argo_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ARGO_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Argo-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Argo auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Argo sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Argo
        ...
        # bundled Argo image, which drops to the `argo` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Argo sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Argo should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``argo chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.argo/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: argo sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Argo truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``argo kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``argo kanban decompose`` and the kanban
        ...
        # ``argo profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `argo model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `argo --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `argo -c` muscle memory.  Default off so existing
        # users aren't surprised.  ARGO_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/argo`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.argo/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.argo/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Argo feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Argo auto-pauses the goal and
    ...
    # always goes to ~/.argo/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ARGO_SKILL_DIR} and ${ARGO_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `argo curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.argo/skills/ into
        # ~/.argo/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `argo curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for argo-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Argo Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ARGO_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.argo/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ARGO_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `argo doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``argo_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Argo to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via ARGO_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `argo -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `argo` launches with
        ...
        # decomposition is manual via `argo kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     argo-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.argo/logs/.
    ...
    # update model picker lists without shipping a argo-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://argo-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `argo model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.argo/state.db.
    ...
    "sessions": {
        ...
        # default of ``argo sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.argo/sessions/session_{sid}.json`` on every turn
    ...
    # ``argo update`` behaviour.
    "updates": {
        # Run a full ``argo backup``-style zip of ARGO_HOME before every
        # ``argo update``.  Backups land in ``<ARGO_HOME>/backups/`` and
        # can be restored with ``argo import <path>``.  Off by default —
        # on large ARGO_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<ARGO_HOME>/lsp/bin/`` on first use.
    ...
    # `argo tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.argo/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.argo/.env (or your shell) and never in config.yaml.
            ...
            # ~/.argo/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'argo model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Argo auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @argo:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ARGO_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #argo)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: argo-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'argo-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Argo API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Argo API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Argo API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ARGO_TOOL_PROGRESS and ARGO_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.argo.config`` entries, then checks
    ...
    try:
        ...
        # should never break `argo update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Argo versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``argo_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'argo setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Argo won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'argo doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ARGO_HOME", "~/.argo")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ARGO_TOOL_PROGRESS")
            old_mode = get_env_value("ARGO_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ARGO_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ARGO_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ARGO_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ARGO_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ARGO_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_argo_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `argo plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `argo curator status` has no
    ...
    #   3. Creates `~/.argo/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_argo_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_argo_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: argo config set <key> <value>")
    ...
    # metadata.argo.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _argo_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.argo/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.argo/config.yaml.
    ...
    (which change ``ARGO_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — argo auth) — OpenAI Codex
#   nous         (OAuth — argo auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.argo/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.argo/.env.
    ...
    interactive menu render (`argo tools`, `argo setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `argo tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Argo env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.argo/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.argo/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.argo/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.argo/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Argo — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Argo vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.argo/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Argo Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  argo config edit     # Edit config file", Colors.DIM))
    print(color("  argo config set <key> <value>", Colors.DIM))
    print(color("  argo setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: argo config set <key> <value>")
            print()
            print("Examples:")
            print("  argo config set model anthropic/claude-sonnet-4")
            print("  argo config set terminal.backend docker")
            print("  argo config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'argo config migrate' to add them")
        ...
        print("  argo config           Show current configuration")
        print("  argo config edit      Open config in editor")
        print("  argo config set <key> <value>   Set a config value")
        print("  argo config check     Check for missing/outdated config")
        print("  argo config migrate   Update config with new options")
        print("  argo config path      Show config file path")
        print("  argo config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``argo config`` UI so users
