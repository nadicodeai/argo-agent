"""Helpers for loading Argo .env files consistently across entrypoints."""
...
# load_argo_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_argo_dotenv().
# Used by setup / `argo model` flows to label detected credentials so
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_argo_dotenv()`` call.  Returns
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `argo setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``argo_cli.config._sanitize_env_lines`` which
    already knows all valid Argo env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from argo_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_argo_dotenv(
    *,
    argo_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Argo environment files with user config taking precedence.

    Behavior:
    - `~/.argo/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(argo_home or os.getenv("ARGO_HOME", Path.home() / ".argo"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Argo reads
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `argo model`
