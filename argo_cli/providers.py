"""
Single source of truth for provider identity in Argo Agent.
...
2. **Argo overlays** — transport type, auth patterns, aggregator flags,
...
# -- Argo overlay ----------------------------------------------------------
# Argo-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ArgoOverlay:
    """Argo-specific provider metadata layered on top of models.dev."""
...
ARGO_OVERLAYS: Dict[str, ArgoOverlay] = {
    "openrouter": ArgoOverlay(
    ...
    "openai-codex": ArgoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": ArgoOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ArgoOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ARGO_QWEN_BASE_URL",
    ),
    "google-gemini-cli": ArgoOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": ArgoOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ArgoOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ArgoOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ArgoOverlay(
    ...
    "minimax-oauth": ArgoOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ArgoOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ArgoOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": ArgoOverlay(
        transport="openai_chat",
    ...
    "opencode": ArgoOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ArgoOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "argo", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Argo overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Argo overlay
    ...
    overlay = ARGO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + argo extra
    ...
    if overlay is not None:
        # Argo-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ARGO_OVERLAYS
    if provider == "bedrock":
