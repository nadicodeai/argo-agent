"""CLI handlers for ``argo secrets bitwarden ...``.
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    Called from ``argo_cli.main`` as part of building the top-level
    ``argo secrets`` parser.
    """
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Argo process."
    )
    console.print(
        "  Status:  [cyan]argo secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]argo secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]argo secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]argo secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Argo will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`argo secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if not args.apply:
        console.print(
            ...
            "next [cyan]argo[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] "
