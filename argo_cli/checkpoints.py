"""`argo checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.argo/checkpoints/``.  Actions:

    argo checkpoints               # same as `status`
    argo checkpoints status        # total size, project count, breakdown
    argo checkpoints list          # per-project checkpoint counts + workdir
    argo checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    argo checkpoints clear [-f]    # nuke the entire base (asks first)
    argo checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    argo checkpoints
    argo checkpoints prune --retention-days 3 --max-size-mb 200
    argo checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: argo checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``argo checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `argo checkpoints` → status
