...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m argo_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Argo_Gateway"
_TASK_DESCRIPTION = "Argo Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Argo profile."""
    from argo_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "argo_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("ARGO_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("ARGO_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("ARGO_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["ARGO_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["ARGO_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["ARGO_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("ARGO_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("ARGO_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("ARGO_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Argo_Gateway``
    Named profile X: ``Argo_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during argo_cli boot.
    from argo_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\argo\\gateway-service\\<task_name>.cmd``
    (or ``<ARGO_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Argo installs stay self-contained).
    """
    _assert_windows()
    from argo_cli.config import get_argo_home

    script_dir = Path(get_argo_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports ARGO_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m argo_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "ARGO_HOME={argo_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "ARGO_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports argo_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "argo_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    argo_home = str(Path(get_argo_home()).resolve())
    profile_arg = _profile_arg(argo_home)

    content = _build_gateway_cmd_script(python_path, working_dir, argo_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Argo builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "argo_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m argo_cli.main gateway run``
    ...
    log_dir = Path(get_argo_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("ARGO_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("ARGO_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from argo_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `argo -p <profile> gateway install` must be safe.
    ...
    from argo_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"argo {profile_arg} gateway start" if profile_arg else "argo gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: argo gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Argo gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: argo gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: argo gateway start")
        _print_next_steps()
        return
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from argo_cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from argo_cli.config import get_argo_home
        print(f"    type {Path(get_argo_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_argo_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from argo_cli.config import get_argo_home

    argo_home = Path(get_argo_home()).resolve()
    print()
    print("Next steps:")
    print("  argo gateway status                      # Check status")
    print(f"  type {argo_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Argo gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: argo gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: argo gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: argo gateway start")
            return
...
def stop() -> None:
    ...
    _assert_windows()
    from argo_cli.gateway import kill_gateway_processes
