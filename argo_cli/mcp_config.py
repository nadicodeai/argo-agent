"""
MCP Server Management CLI — ``argo mcp`` subcommand.

Implements ``argo mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.argo/config.yaml under the ``mcp_servers`` key.
"""
...
from argo_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_argo_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from argo_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # argo_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  argo mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  argo mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  argo mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_argo_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: argo mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_argo_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `argo mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  argo mcp add <name> --url <endpoint>')
        _info('  argo mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `argo mcp remove` + `argo mcp add` to reconfigure auth.")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'argo mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``argo mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("argo mcp serve                              Run as MCP server")
        _info("argo mcp add <name> --url <endpoint>        Add an MCP server")
        _info("argo mcp add <name> --command <cmd>         Add a stdio server")
        _info("argo mcp add <name> --preset <preset>       Add from a known preset")
        _info("argo mcp remove <name>                      Remove a server")
        _info("argo mcp list                               List servers")
        _info("argo mcp test <name>                        Test connection")
        _info("argo mcp configure <name>                   Toggle tools")
        _info("argo mcp login <name>                       Re-authenticate OAuth")
        print()
