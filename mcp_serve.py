"""
Argo MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Argo-specific extra)

Usage:
    argo mcp serve
    argo mcp serve --verbose
...
logger = logging.getLogger("argo.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ARGO_HOME."""
    try:
        from argo_constants import get_argo_home
        return get_argo_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ARGO_HOME", Path.home() / ".argo")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from argo_constants import get_argo_home
        directory_file = get_argo_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ARGO_HOME", Path.home() / ".argo")
...
class EventBridge:
    ...
    This is the Argo equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from argo_constants import get_argo_home
            db_file = get_argo_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ARGO_HOME", Path.home() / ".argo")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Argo MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "argo",
        instructions=(
            "Argo Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Argo MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
